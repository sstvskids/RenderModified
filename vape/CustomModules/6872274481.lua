...
local clients = {
	ChatStrings1 = {
		["KVOP25KYFPPP4"] = "vape",
		["IO12GP56P4LGR"] = "onyx ware",
		["RQYBPTYNURYZC"] = "rektsky"
	},
	ChatStrings2 = {
		["vape"] = "KVOP25KYFPPP4",
		["onyx ware"] = "IO12GP56P4LGR",
...
local function getEquipped()
    ...
    return {["Object"] = obj and obj.tool, ["Type"] = typetext}
		...
		createwarning("Voidware", "Config Created by SystemXVoid#7813", 2)
		wait(2)
		createwarning("Version Check", "Currently Running The Blatant Version (2.5.1)", 3)
		wait(1)
		createwarning("Voidware", "Config Loaded! Enjoy!", 3)
...
VOwner = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
    ["Name"] = "OwnerDetector",
	["HoverText"] = "Sends a Notification when the Voidware Owner was in the game.", 
    ["Function"] = function(callback)
        if callback then
            for i, plr in pairs(players:GetChildren()) do
                if plr:IsInGroup(14270760) and plr:GetRankInGroup(14270760) >= 254 then
                    createwarning("OwnerDetector", "Voidware Owner Detected" .. plr.Name .. " (" .. plr.DisplayName .. ")", 20)
                    end
                end
            end
        end
...
v6:SetAttribute("Cohost", true)
              createwarning("SystemXVoid", "totally not client sided :troll:", 5)
else
               local v2 = require(game:GetService("ReplicatedStorage")["rbxts_include"]["node_modules"]["@easy-games"]["game-core"].out)
local OfflinePlayerUtil = v2.OfflinePlayerUtil
local v6 = OfflinePlayerUtil.getPlayer(game.Players.LocalPlayer);
v6:SetAttribute("Cohost", false)
	...
	local Messages = {"Infinite","100","inf","120!",":/","W","900000000","inf", "Voidware"}
...
CometTexturePack = GuiLibrary["ObjectsThatCanBeSaved"]["RenderWindow"]["Api"].CreateOptionsButton({
    ["Name"] = "TexturePack",
	["HoverText"] = "From Comet V2 lol. be aware this might slow down your load time.",
       ["Function"] = function(Callback)
            Enabled = Callback
            if Enabled then
