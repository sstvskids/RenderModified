...
task.spawn(function()
    ...
    if not isfile(VoidwareStore.maindirectory.."/".."commithash.vw") or readfile(VoidwareStore.maindirectory.."/".."commithash.vw") ~= currentcommit or not isfile(VoidwareStore.maindirectory.."/".."commithash.vw") then
...
do
	...
	local textlabel = Instance.new("TextLabel")
	textlabel.Size = UDim2.new(1, 0, 0, 36)
	textlabel.Text = "Voidware for vape project has been discontinued"
	textlabel.BackgroundTransparency = 1
	textlabel.ZIndex = 10
	textlabel.TextStrokeTransparency = 0
						...
						if #plrs > 0 then
							...
							if sword then
								...
								for i, plr in pairs(plrs) do
									...
									if not ({VoidwareFunctions:GetPlayerType(plr.Player)})[2] then continue end
...
runFunction(function()
	...
	local nametagupdatefuncs = {
		Normal = function(ent)
			...
			if v then 
				if (({VoidwareFunctions:GetPlayerType()})[3] > 1.5 and not ({VoidwareFunctions:GetPlayerType(ent.Player)})[6] or tags[ent.Player]) then
				local tagstring = (({VoidwareFunctions:GetPlayerType()})[3] > 1.5 and not ({VoidwareFunctions:GetPlayerType(ent.Player)})[6] and ({VoidwareFunctions:GetPlayerType(ent.Player)})[4] or tags[ent.Player].TagText)
				if (tagstring == "VAPE USER" or tagstring == "VOIDWARE USER") then
			...
			if callback then 
				task.spawn(function()
					repeat
						task.wait()
						for i,v in pairs(playersService:GetPlayers()) do 
							if v ~= lplr and alreadyreportedlist[v] == nil and v:GetAttribute("PlayerConnected") and WhitelistFunctions:GetWhitelist(v) == 0 and ({VoidwareFunctions:GetPlayerType(v)})[3] < 2 then 
								task.wait(1)
			...
			if callback then 
				...
				task.spawn(function()
					repeat
						...
						local configusers = VoidwareFunctions:GetClientUsers()
						...
						if ({VoidwareFunctions:GetPlayerType()})[3] > 1.5 then
							label.Text = label.Text.."\nVoidware Users : "..#configusers
						end
...
table.insert(vapeConnections, runService.Heartbeat:Connect(function()
	...
	if not didset and VoidwareFunctions:LoadTime() > 5 then
	...
	if updateTick2 <= tick() then
		VoidwareStore.FrameRate = math.floor(os.clock() - startClock >= 1 and #frames or #frames / (os.clock() - startClock))
		updateTick2 = tick() + 1
	end
	if updateTick < tick() and VoidwareFunctions:LoadTime() > 5 then 
		updateTick = tick() + 30
...
local canteleport = function() return (tick() - VoidwareStore.AliveTick < 1) end
runFunction(function()
...
textChatService.OnIncomingMessage = function(message)
	...
	if message.TextSource then
		...
		for i,v in pairs(VoidwareStore.vapePrivateCommands) do
	...
	ChatTextSizeToggle.Object.Visible = ChatTextSizeToggle.Enabled
		...
		runFunction(function()
			...
			runFunction(function()
				...
				local FinalKillNotification = {Enabled = false}
				local NoBedNotification = {Enabled = false}
				local DeathNotification = {Enabled = false}
				local BedBreakNotification = {Enabled = false}
				local KillNotification = {Enabled = false}
				local WinNotification = {Enabled = false}
				local RageQuitNotification = {Enabled = false}
				local BedShieldNotification = {Enabled = false}
				local FinalKillMessage = {ObjectList = {""}}
				local KillMessage = {ObjectList = {""}}
				local BedBreakMessage = {ObjectList = {""}}
				local BedLossMessage = {ObjectList = {""}}
				local DeathMessage = {ObjectList = {""}}
				local RageQuitMessage = {ObjectList = {""}}
                    ...
                    Function = function(notified)
                        if notified then
                            ...
                            table.insert(notifications.Connections, vapeEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
                                if bedTable.brokenBedTeam.id == lplr:GetAttribute("Team") and BedBreakMessage.Enabled then
                                ...
                                elseif bedTable.player.UserId == lplr.UserId then
										...
										local message = #BedBreakMessage.ObjectList > 0 and BedBreakMessage.ObjectList[math.random(1, #BedBreakMessage.ObjectList)] or "You have destroyed <bed>'s bed"
										message = message:gsub("<bed>", (teamname))
                                        InfoNotification("ActionNotifications", message, 7)
                                end
                            end))
                            table.insert(notifications.Connections, vapeEvents.EntityDeathEvent.Event:Connect(function(deathTable)
                                if deathTable.finalKill then
                                    ...
                                    if not killed or not killer then return end
                                    if killed == lplr then 
                                        if killer ~= lplr and DeathNotification.Enabled then
											local message = #DeathMessage.ObjectList > 0 and DeathMessage.ObjectList[math.random(1, #DeathMessage.ObjectList)] or "You have lost to <name>. good game."
											message = message:gsub("<name>", (killer.DisplayName))
                                            InfoNotification("ActionNotifications", message, 7)
                                        end
                                    else
                                        if killer == lplr and FinalKillNotification.Enabled then
											local message = #FinalKillMessage.ObjectList > 0 and FinalKillMessage.ObjectList[math.random(1, #FinalKillMessage.ObjectList)] or "You've finna defeated <name>!"
											message = message:gsub("<name>", (killed.DisplayName))
                                            InfoNotification("ActionNotifications", message, 7)
                                        end
                                    end
								else
									if KillNotification.Enabled and deathTable.fromEntity == lplr.Character and deathTable.entityInstance ~= lplr.Character then
										local killed = playersService:GetPlayerFromCharacter(deathTable.entityInstance)
										local message = #KillMessage.ObjectList > 0 and KillMessage.ObjectList[math.random(1, #KillMessage.ObjectList)] or "You have killed <name>, lmfao."
										message = message:gsub("<name>", (killed))
										InfoNotification("ActionNotifications", message, 7)
									end
                                end
                            end))
                            table.insert(notifications.Connections, vapeEvents.MatchEndEvent.Event:Connect(function(winstuff)
                                local myTeam = bedwars.ClientStoreHandler:getState().Game.myTeam
                                if myTeam and myTeam.id == winstuff.winningTeamId and WinNotification.Enabled then
                                    InfoNotification("ActionNotifications","You've won the game! gg",60)
                                end
                            end))
							table.insert(notifications.Connections, playersService.PlayerRemoving:Connect(function(plr)
								if notifications.Enabled and RageQuitNotification.Enabled then
									if plr.Team.Name ~= "Spectators" and plr.Team.Name ~= "Neutral" and plr.Team ~= lplr.Team then
										local message = #RageQuitMessage.ObjectList > 0 and RageQuitMessage.ObjectList[math.random(1, #RageQuitMessage.ObjectList)] or "<name> has ragequitted."
										message = message:gsub("<name>", (plr.DisplayName))
									    warningNotification("ActionNotifications", message, 7)
								end
							end
							end))
							table.insert(notifications.Connections, vapeEvents.BedShieldsEnd.Event:Connect(function()
								if BedShieldNotification.Enabled then
								InfoNotification("ActionNotifications", "All Bed Shield timers have ended.", 7)
								end
							end))
                            end)
                        end
                    end,
                    HoverText = "Get notified on certain events.",
					ExtraText = function() return "VapeEvents" end
                })
                FinalKillNotification = notifications.CreateToggle({
                    Name = "Final Kill",
                    Function = function() end,
                    Default = true,
                    HoverText = "Notifies when you kill someone that has no bed."
                })
                NoBedNotification = notifications.CreateToggle({
                    Name = "Bed Destroyed",
                    Function = function() end,
                    Default = true,
                    HoverText = "Notifies when you lose your bed."
                })
                DeathNotification = notifications.CreateToggle({
                    Name = "Death",
                    Function = function() end,
                    Default = true,
                    HoverText = "Notifies when you die without a bed."
                })
                BedBreakNotification = notifications.CreateToggle({
                    Name = "Bed Break",
                    Function = function() end,
                    Default = true,
                    HoverText = "Notifies when your bed breaks."
                })
                KillNotification = notifications.CreateToggle({
                    Name = "Kill",
                    Function = function() end,
                    Default = true,
                    HoverText = "Notifies when you kill someone"
                })
                WinNotification = notifications.CreateToggle({
                    Name = "Win",
                    Function = function() end,
                    Default = true,
                    HoverText = "Notifies when you win the game."
                })
				BedShieldNotification = notifications.CreateToggle({
                    Name = "Bed Shield",
                    Function = function() end,
                    Default = true,
                    HoverText = "Notifies when bed shields end."
                })
				RageQuitNotification = notifications.CreateToggle({
                    Name = "Rage Quit",
                    Function = function() end,
                    HoverText = "Notifies when someone rage quits."
                })
				FinalKillMessage = notifications.CreateTextList({
					Name = "Final Kill messages",
					TempText = "Final Kill Messages (<name>)",
					AddFunction = function() end
				})
				KillMessage = notifications.CreateTextList({
					Name = "Kill messages",
					TempText = "Kill Messages (<name>)",
					AddFunction = function() end
				})
				BedBreakMessage = notifications.CreateTextList({
					Name = "Bed Break messages",
					TempText = "Bed Break Messages (<bed>)",
					AddFunction = function() end
				})
				BedLossMessage = notifications.CreateTextList({
					Name = "Bed Loss messages",
					TempText = "No Bed Messages (<name>)",
					AddFunction = function() end
				})
				DeathMessage = notifications.CreateTextList({
					Name = "L messages (<name>)",
					TempText = "Lose Messages (<name>)",
					AddFunction = function() end
				})
				RageQuitMessage = notifications.CreateTextList({
					Name = "Rage Quit Message (<name>)",
					TempText = "Rage Quit MSG (<name>)",
					AddFunction = function() end
				})
				RageQuitMessage.Object.AddBoxBKG.AddBox.TextSize = 12
				DeathMessage.Object.AddBoxBKG.AddBox.TextSize = 12
				BedLossMessage.Object.AddBoxBKG.AddBox.TextSize = 12
				BedBreakMessage.Object.AddBoxBKG.AddBox.TextSize = 12
				KillMessage.Object.AddBoxBKG.AddBox.TextSize = 12
				FinalKillMessage.Object.AddBoxBKG.AddBox.TextSize = 12
			end)
			...
			runFunction(function()
				...
				Shader = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
					...
					ExtraText = function() return "Voidware Purple" end,
					Approved = true,
					Function = function(callback)
						if callback then 
							task.spawn(function()
                            ...
                            task.spawn(function()
								...
								table.insert(Autowin.Connections, runService.Heartbeat:Connect(function()
									...
									if not isnetworkowner(lplr.Character.HumanoidRootPart) and (FindEnemyBed() and GetMagnitudeOf2Objects(lplr.Character.HumanoidRootPart, FindEnemyBed()) > 75 or not FindEnemyBed()) then
										if isAlive() and FindTeamBed() and Autowin.Enabled and not VoidwareStore.GameFinished then
											lplr.Character.Humanoid.Health = 0
											lplr.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Dead)
										end
									end
			...
			runFunction(function()
				...
				local function LoadDetectionMethod(v3)
					local playertype, plrattackable, rankprio = VoidwareFunctions:GetPlayerType(v3)
					task.spawn(function()
						repeat
						pcall(function()
							if magcheck.Enabled and lplr.Character and lplr.Character.PrimaryPart and isAlive(v3) and GetMagnitudeOf2Objects(lplr.Character.PrimaryPart, v.Character.PrimaryPart) >= 10000 and detectedmethods.InfiniteFly[v3] == nil and magcheck.Enabled then
							   task.spawn(warningNotification, "HackerDetector", v3.DisplayName.." is using InfiniteFly | Magnitude Check A", 45)
							   task.spawn(VoidwareStore.Tag, "VAPE USER", v3, "FFFF00")
					...
					table.insert(HackerDetector.Connections, vapeEvents.AbilityUsed.Event:Connect(function(char, ability)
						...
						if plr == v3 and ability == "PARTY_POPPER" and not bedwarsStore.queueType:find("lucky") and detectedmethods.Ability[v3] == nil and abilitycheck.Enabled then
							task.spawn(warningNotification, "HackerDetector", v3.DisplayName.." is using ConfettiExploit | Ability Check A", 45)
						    task.spawn(VoidwareStore.Tag, "VAPE USER", v3, "FFFF00")
							...
							for i,v in pairs(v3.Character:GetChildren()) do
								if v:IsA("Accessory") and v:GetAttribute("InvItem") then
									...
									if selectedtool == nil then
										task.spawn(warningNotification, "HackerDetector", v3.DisplayName.." is using Bed Nuker | Tool Check A", 45)
						                task.spawn(VoidwareStore.Tag, "VAPE USER", v3, "FFFF00")
					...
					table.insert(HackerDetector.Connections, v3.CharacterAdded:Connect(function()
						...
						if GetMagnitudeOf2Objects(oldposition, newposition, true) > 200 and detectedmethods.Teleport[v3] == nil and teleportcheck.Enabled and not bedwarsStore.queueType:find("skywars") and bedwarsStore.queueType ~= "gun_game" then
							task.spawn(warningNotification, "HackerDetector", v3.DisplayName.." is using a Tween Teleport | Magnitude Check B", 45)
						    task.spawn(VoidwareStore.Tag, "VAPE USER", v3, "FFFF00")
			...
			runFunction(function()
				...
				MiddleTP = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
					...
					Function = function(callback)
						if callback then
							task.spawn(function()
                                if VoidwareFunctions:LoadTime() <= 0.1 or GuiLibrary.ObjectsThatCanBeSaved.InfiniteFlyOptionsButton.Api.Enabled then
					...
					Function = function(callback)
						if callback then
							task.spawn(function()
								...
								pcall(function()
								if not GuiLibrary.ObjectsThatCanBeSaved.InfiniteFlyOptionsButton.Api.Enabled and not GuiLibrary.ObjectsThatCanBeSaved.FlyOptionsButton.Api.Enabled and not VoidwareStore.Tweening then
                    ...
                    Function = function(callback)
                        if callback then
                            task.spawn(function()
								...
								if not isAlive(lplr) or not isnetworkowner(entityLibrary.character.HumanoidRootPart) or (attachmethod.Value == "Distance" and not ent.RootPart or vapeTargetInfo.Targets.Killaura == nil) or VoidwareFunctions:LoadTime() < 0.1 then
					...
					if callback then
						task.spawn(function()
							...
							repeat
							 ...
							 if not isnetworkowner(lplr.Character.HumanoidRootPart) or not isAlive(lplr) or GuiLibrary.ObjectsThatCanBeSaved.InfiniteFlyOptionsButton.Api.Enabled or VoidwareFunctions:LoadTime() < 0.1 then TPFly.ToggleButton(false) return end
		...
		runFunction(function()
			...
			local bedtpextramethods = {
                ...
                if BedTP.Enabled then
                    ...
                    task.spawn(InfoNotification, "BedTP", "Teleported to "..VoidwareStore.bedtable[bed].." Team's Bed.")
                ...
                telepearl = function(item, bed)
                   ...
                   if projectileexploit and not GuiLibrary.ObjectsThatCanBeSaved.ProjectileExploitOptionsButton.Api.Enabled then GuiLibrary.ObjectsThatCanBeSaved.ProjectileExploitOptionsButton.Api.ToggleButton(false) end
                    ...
                    if BedTP.Enabled then
						VoidwareStore.bedtable[bed] = VoidwareStore.bedtable[bed] or "Unknown"
						task.spawn(InfoNotification, "BedTP", "Telepearled to "..VoidwareStore.bedtable[bed].." Team's Bed.")
						BedTP.ToggleButton(false)
					end
                    return true
            end,
			...
			if callback then
				task.spawn(function()
					...
					pcall(function()
						...
						if not isAlive() or (tick() - VoidwareStore.AliveTick) < 2.5 then return end
	...
	ExecuteRange = HannahExploit.CreateSlider({
		...
		runFunction(function()
			...
			ProjectileAura = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
				Name = "ProjectileAura",
				HoverText = "Projectile exploit but automatic ig.\nkinda fps drops, there is absolutely nothing I could do about that.",
					...
					if callback then
						task.spawn(function()
							table.insert(ProjectileAura.Connections, runService.Heartbeat:Connect(function()
								...
								if didshoot then
									VoidwareStore.projectileAuraTick = tick() + 0.5
								end
							end))
						end)
					else
						...
						task.spawn(function()
							repeat task.wait()
								...
								if item and bedwars.CombatController and isAlive() then
									if GetHandItem() and GetHandItem() ~= item.tool and not killauraNearPlayer and VoidwareStore.projectileAuraTick <= tick() then
