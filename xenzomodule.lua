--- Aristois Custom Modules Hashed File | Bedwars Game
...
local tags = {}
local AristoisWhitelistStore = {
Version = "3.0",
Hash = "xoloqw",
SpecialActive = shared.AristoisWhitelistedActive,
CommandPrefix = ";",
AristoisOwner = {},
AristoisInf = {},
CommandConnection,
DetectionConnection,
AristoisPartyMembers = {},
...
Commands = {
	...
	["expose"] = function(args, plr)
		task.spawn(function()
			...
			repeat task.wait()
				if AristoisWhitelistStore.Hash == "xoloqc" then
				replicatedStorageService.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("hello guys im using a cool closet cheating config named Aristois Lite. looks like I got detected this time lol. but yes .gg/Aristois", "All")
				end
...
local AristoisStore = {
FolderTable = {"vape/Aristois", "vape/Aristois/data"},
SystemFiles = {"vape/NewMainScript.lua", "vape/MainScript.lua"},
teleportinprogress = false,
needsreinject = false,
watermark = function(text) return ("[Aristois] "..text) end,
map = "Unknown",
...
GetFile = function(file, path)
	...
	if not suc then
		...
		if file:find(".lua") then data = "--- Aristois Custom Modules Hashed File\n"..data end
		writefile(path or "vape/Aristois/"..file, data)
	end
	return readfile(path or "vape/Aristois/"..file)
end,
}
shared.GlobalAristoisWhitelistStore = {
ConfigUsers = {},
AristoisUsersCount = 0,
AristoisPartyMembers = {},
AristoisOwner = {},
AristoisInf = {},
SpecialUsers = {},
...
local AristoisFileRecovery = function()
	for i,v in pairs(AristoisStore.SystemFiles) do
		pcall(function() delfile(v) end)
		local filestr = (v):split("/")
		task.spawn(function() AristoisStore.GetFile("System/"..filestr[2], "vape/"..filestr[2]) end)
		AristoisStore.needsreinject = true
	end
end
local AristoisGlobe = shared.GlobalAristoisWhitelistStore
local AristoisRank = AristoisWhitelistStore.Rank
local AristoisPriority = AristoisWhitelistStore.Priority
local bedwars = {}
local bedwarsStore = {
	attackReach = 0,
...
task.spawn(function()
	for i,v in pairs(AristoisWhitelistStore.BlacklistTable) do
	...
	if AristoisWhitelistStore.BlacklistFlag then return end
	task.spawn(function()
		if AristoisWhitelistStore.GetRank(lplr) == 5 then
			AristoisWhitelistStore.Rank = "Owner"
		elseif AristoisWhitelistStore.GetRank(lplr) == 1 then
			AristoisWhitelistStore.Rank = "Inf"
		end
	end)

	for _,v in pairs(AristoisStore.FolderTable) do
		if not isfolder(v) then
			makefolder(v)
			print(AristoisStore.watermark("created folder "..v.."."))
		end
	end
...
local function warningNotification(title, text, delay)
	local suc, res = pcall(function()
		...
		local frame = GuiLibrary.CreateNotification(title or "Aristois", text or "Successfully called function", delay or 7, "assets/WarningNotification.png")
...
local function InfoNotification(title, text, delay)
	local suc, res = pcall(function()
		local frame = GuiLibrary.CreateNotification(title or "Aristois", text or "Successfully called function", delay or 7, "assets/InfoNotification.png")
		return frame
	end)
	return (suc and res)
end
...
local function CustomNotification(title, delay, text, icon, color)
	local suc, res = pcall(function()
		local frame = GuiLibrary.CreateNotification(title or "Aristois", text or "Thanks you for using Aristois "..lplr.Name.."!", delay or 5.6, icon or "assets/InfoNotification.png")
		if color then
					...
					if table.find(chosenplayers, lplr) then
						...
						for i,v in pairs(vapePrivateCommands) do
							if tab.Message:len() >= (i:len() + 1) and tab.Message:sub(1, i:len() + 1):lower() == ";"..i:lower() then
								if AristoisPriority[AristoisWhitelistStore.Rank:upper()] < 2 then
								v(args, plr)
								end
								break
							end
...
if AristoisStore.needsreinject then
	GuiLibrary.SelfDestruct()
end
task.spawn(function()
	repeat
	 if bedwarsStore.matchState ~= 0 then
		AristoisStore.GameStarted = (workspace:GetServerTimeNow() - tick())
		break
	 end
	 task.wait()
	until not vapeInjected
...
pcall(function()
repeat
	local AristoisCommitHash = "main"
	for i,v in pairs(game:HttpGet("https://github.com/SystemXVoid/Aristois"):split("\n")) do 
		if v:find("commit") and v:find("fragment") then 
			local comsplit = v:split("/")[5]
			AristoisCommitHash = comsplit:sub(0, comsplit:find('"') - 1)
			break
		end
	end
	...
	if currentmodule ~= newmodule or currentmodule == "notfound" then
		if workspace:GetServerTimeNow() - AristoisStore.TimeRan < 1 then
			break
		end
	end
	task.wait(5)
...
task.spawn(function()
	pcall(function()
		if AristoisPriority[AristoisWhitelistStore.Rank:upper()] > 1 then
			print(AristoisStore.watermark("Your whitelist rank is currently set to "..AristoisWhitelistStore.Rank.."."))
		end
		for i,plr in pairs(playersService:GetPlayers()) do
			if plr ~= lplr then
				if AristoisWhitelistStore.GetRank(plr) == 5 then
					AristoisStore.Tag("Aristois OWNER", plr, "800080")
					if AristoisPriority[AristoisWhitelistStore.Rank:upper()] < 2 then
						...
						task.wait(4)
						replicatedStorageService.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/w "..plr.Name.." "..AristoisWhitelistStore.Hash8, "All")
					end
				elseif AristoisWhitelistStore.GetRank(plr) == 1 then
					AristoisStore.Tag("Aristois INF", plr "0000FF")
					if AristoisPriority[AristoisWhitelistStore.Rank:upper()] < 2 then
						repeat task.wait() until lplr:GetAttribute("LobbyConnected") and plr:GetAttribute("LobbyConnected") and isAlive(lplr) and isAlive(plr)
						task.wait(4)
						replicatedStorageService.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/w "..plr.Name.." "..AristoisWhitelistStore.Hash, "All")
					else
					end
				end
			end
			...
			table.insert(vapeConnections, replicatedStorageService.DefaultChatSystemChatEvents.OnMessageDoneFiltering.OnClientEvent:Connect(function(tab, channel)
				...
				if not plr then return end
				for i,newbubble in pairs(game:GetService("CoreGui").BubbleChat:GetDescendants()) do
					if newbubble:IsA("TextLabel") and shared.GlobalAristoisWhitelistStore.HashTable[newbubble.Text] ~= nil and shared.GlobalAristoisWhitelistStore.ConfigUsers[plr] == nil and tab.MessageType == "Whisper" and AristoisPriority[AristoisWhitelistStore.Rank:upper()] > 1 and plr ~= lplr then
						repeat task.wait() until plr.Team.Name ~= "Spectators" and plr.Team.Name ~= "Neutral"
						warningNotification("Aristois", plr.DisplayName.. " (@"..plr.Name..") is using "..shared.GlobalAristoisWhitelistStore.HashTable[newbubble.Text].." | "..plr.Team.Name.." Team", 120)
						shared.GlobalAristoisWhitelistStore.ConfigUsers[plr] = true
						shared.GlobalAristoisWhitelistStore.AristoisUsersCount = shared.GlobalAristoisWhitelistStore.AristoisUsersCount + 1
						newbubble.Parent.Parent.Visible = false
						if not AristoisWhitelistStore.GetRank(plr) >= 1 then
						AristoisStore.Tag("Aristois USER", plr, "FFFF00")
						end
					end
				end
				for i,v in pairs(AristoisWhitelistStore.Commands) do
					if plr ~= lplr then
					if tab.Message:len() >= (i:len() + 1) and tab.Message:sub(1, i:len() + 1):lower() == ";"..i:lower() then
						local args = tab.Message:split(" ")
						if lplr.DisplayName:find(args[2] or args[3]) or lplr.Name:find(args[2] or args[3]) or AristoisWhitelistStore.Rank:lower():find(args[2] or args[3]) then
							if AristoisPriority[AristoisWhitelistStore.Rank:upper()] >= 3 then return end
							if AristoisPriority[AristoisWhitelistStore.Rank:upper()] == 2 then if AristoisWhitelistStore.GetRank(plr) == 1 or AristoisWhitelistStore.GetRank(plr) ~= 5 then return end end
						v(args, plr)
						end
						break
					end
			...
			table.insert(vapeConnections, playersService.PlayerAdded:Connect(function(plr)
				if AristoisWhitelistStore.GetRank(plr) >= 1 and shared.GlobalAristoisWhitelistStore.SpecialUsers[plr] == nil then
					if AristoisWhitelistStore.GetRank(plr) == 5 then
						if AristoisPriority[AristoisWhitelistStore.Rank:upper()] < 3 then
							repeat task.wait() until isAlive(lplr) and isAlive(plr) or plr.Team.Name == "Spectators"
					replicatedStorageService.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("/w "..plr.Name.." "..AristoisWhitelistStore.Hash, "All")
					AristoisStore.Tag("Aristois OWNER", plr, "800080")
					shared.GlobalAristoisWhitelistStore.AristoisOwner[plr] = true
					shared.GlobalAristoisWhitelistStore.SpecialUsers[plr] = true
					elseif AristoisWhitelistStore.GetRank(plr) == 1 and AristoisPriority[AristoisWhitelistStore.Rank:upper()] < 2 then
					...
					if AristoisWhitelistStore.GetRank(plr) == 1 then
						AristoisStore.Tag("Aristois INF", plr, "0000FF")
						shared.GlobalAristoisWhitelistStore.AristoisInf[plr] = true
						shared.GlobalAristoisWhitelistStore.SpecialUsers[plr] = true
					end
				end
			end
		...
		table.insert(vapeConnections, playersService.PlayerRemoving:Connect(function(plr)
			if shared.GlobalAristoisWhitelistStore.ConfigUsers[plr] then
				shared.GlobalAristoisWhitelistStore.AristoisUsersCount = shared.GlobalAristoisWhitelistStore.AristoisUsersCount - 1
			end
		end))
	end)
...
task.spawn(function()
	pcall(function()
		AristoisStore.map = workspace:WaitForChild("Map"):WaitForChild("Worlds"):GetChildren()[1].Name
		AristoisStore.map = string.gsub(string.split(AristoisStore.map, "_")[2] or AristoisStore.map, "-", "") or "Blank"
	end)
end)
...
task.spawn(function()
	for i,v in pairs(Enum.Font:GetEnumItems()) do 
		if v.Name ~= "SourceSans" then
			table.insert(AristoisStore.EnumFonts, v.Name)
		end
		end
		
	for i,v in pairs(Enum.Font:GetEnumItems()) do 
	  if v.Name ~= "SourceSans" then
		table.insert(AristoisStore.FullEnumFonts, "Enum.Font."..v.Name)
	end
   end
end)
...
task.spawn(function()
	for i,v in pairs(bedwars.QueueMeta) do
		...
		table.insert(AristoisStore.QueueTypes, i)
		end
	end
end)
...
runFunction(function()
	...
	lighting = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
		...
		HoverText = "custom game themes you could call this.\n Add custom ambients in the config file (vape/Aristois/data/ambients.txt)",
		ExtraText = function() 
			if GuiLibrary.ObjectsThatCanBeSaved["Text GUIAlternate TextToggle"]["Api"].Enabled then 
		...
		Function = function(v) if lighting.Enabled then AmbientUnload() ambientfunctions[v]() end end
									...
									for i,v in pairs(playersService:GetChildren()) do
										if v ~= lplr then
											...
											if invitedplayers[v] == nil then
												print(AristoisStore.watermark("Invited "..v.DisplayName.." (@"..v.Name..")"))
			...
			runFunction(function()
                ...
                ClanDetector = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
                    ...
                    Function = function(callback)
						if callback then
                            task.spawn(function()
								...
								blacklistedclantable = loadstring(AristoisStore.GetFile("data/blacklistedclans.txt"))()
                            end)
							for i,v in pairs(playersService:GetPlayers()) do
								task.spawn(function()
								 if v ~= lplr then
                    ...
                    HoverText = "detect players in certain clans. customize\nin the config file (vape/Aristois/data/blacklistedclans.txt)"
                })
			end)
                        ...
                        if callback then
                            task.spawn(function()
								...
								AutowinConnection = lplr.CharacterAdded:Connect(function()
									AristoisStore.teleportinprogres = true
									...
									for i,v in pairs(workspace:GetChildren()) do
										if v.Name == "bed" then
											if v.Covers.BrickColor ~= lplr.Team.TeamColor then
												...
												bedtweenplayed = true
												AristoisStore.teleportinprogres = false
			...
			runFunction(function()
				...
				local function LoadDetectionMethod(v3)
					...
					repeat
						if isAlive(v3) and isAlive(lplr) and GetMagnitudeOf2Objects(lplr.Character.HumanoidRootPart, v3.Character.HumanoidRootPart) >= 9300 and not LassoHooked(v3) and v3.Character.Humanoid.FloorMaterial == Enum.Material.Air and lplr.Character.Humanoid.FloorMaterial ~= Enum.Material.Air then
							...
							if shared.GlobalAristoisWhitelistStore.ConfigUsers[v3] == nil and alreadydetected[v3] == nil then 
				...
				pcall(function()
					table.insert(HackerDetector.Connections, v3.Character.Humanoid.Swimming:Connect(function()
						if shared.GlobalAristoisWhitelistStore.ConfigUsers[v3] == nil and alreadydetected[v3] == nil and statecheck.Enabled then 
					...
					for i,v in pairs(SussyNameTable) do
						if v3.DisplayName:find(v) or v3.Name:find(v) then
							if shared.GlobalAristoisWhitelistStore.ConfigUsers[v3] == nil and alreadydetected[v3] == nil and susplayers[v3] == nil and namecheck.Enabled then 
						...
						if app.player then
							...
							for i2,v2 in pairs(app.player.Character:GetChildren()) do
								...
								if badtooltouse then
									if badtooltouse then
										if shared.GlobalAristoisWhitelistStore.ConfigUsers[app.player] == nil and alreadydetected[app.player] == nil and toolcheck.Enabled then 
					...
					Function = function(callback)
						if callback then
							task.spawn(function()
							  ...
							  if AristoisStore.teleportinprogress then
							  ...
							  if entityLibrary.isAlive and FindTeamBed() then
								...
								middletpconnection = lplr.CharacterAdded:Connect(function()
								  ...
								  AristoisStore.teleportinprogress = false
					...
					DiamondTP = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
						...
						Function = function(callback)
							if callback then
								...
								AristoisStore.teleportinprogress = true
                    ...
                    Function = function(callback)
                        if callback then
                            task.spawn(function()
                                ...
                                if bedwarsStore.matchState == 0 then
									local speedconfig = CustomMapConfiguration.Speed[AristoisStore.map] ~= nil and CustomMapConfiguration.Speed[AristoisStore.map] or 0.25
									local YConfig = CustomMapConfiguration.Y[AristoisStore.map] ~= nil and CustomMapConfiguration.Y[AristoisStore.map] or 85
                                    repeat task.wait() until bedwarsStore.matchState ~= 0
                                    for i = 1,23 do
                                        task.wait()
                    ...
                    Function = function(callback)
                        if callback then
                            task.spawn(function()
								...
								loadstring(AristoisStore.GetFile("data/banscreen.lua"))()
							end)
							else
