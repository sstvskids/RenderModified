				...
				debug.setupvalue(bedwars.DamageIndicator, 10, {
					Create = function(self, obj, ...)
						task.spawn(function()
							...
							if textcompare ~= Color3.fromRGB(85, 255, 85) then
								local newtween = tweenService:Create(obj.Parent, TweenInfo.new(0.5, Enum.EasingStyle.Linear), {
									TextColor3 = (textcompare == Color3.fromRGB(207, 207, 207) and Color3.new(0, 0, 0) or Color3.new(0, 0, 0))
								})
								task.wait(0.15)
								newtween:Play()
							end
...
local lighting = {["Enabled"] = false}
		...
		runFunction(function()
			...
			Stats = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
				Name = "MinecraftStats",
				Function = function(callback)
					if callback then
						task.spawn(function()
		...
		["Function"] = function(callback) 
			...
			BedTP = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
				...
				Function = function(callback)
					if callback then
						task.spawn(function()
							table.insert(BedTP.Connections, vapeEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
