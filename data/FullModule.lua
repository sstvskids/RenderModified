	...
	task.spawn(function()
		...
		local function warningNotification(title, text, delay)
			local suc, res = pcall(function()
				local frame = GuiLibrary.CreateNotification(title or "Voidware", text or "Thanks you for using Voidware"..lplr.Name.."!", delay or 5.6, "assets/WarningNotification.png")
		...
		runFunction(function()
			...
			local anims = {
				...
				["FunnyV2"] = {
					{CFrame = CFrame.new(0, 6, 0) * CFrame.Angles(math.rad(0), math.rad(0), math.rad(0)),Time = 0.25},
					{CFrame = CFrame.new(0, 0, 0) * CFrame.Angles(math.rad(0), math.rad(0), math.rad(0)), Time = 0.10}
				},
				["Smooth"] = {
			...
			killauraanimmethod = Killaura.CreateDropdown({
				Name = "Animation", 
				List = {"Normal", "Slow", "New", "Vertical Spin", "Exhibition", "Exhibition Old","Funny","FastV2","FunnyV2","Smooth"},
				Function = function(val) end
			})
			...
			killauraparticle = Killaura.CreateToggle({
				...
				Function = function(callback)
					if callback then 
						...
						repeat task.wait()
						particle.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Color3.fromHSV(critpartcolor.Hue, critpartcolor.Sat, critpartcolor.Value)), ColorSequenceKeypoint.new(1, Color3.fromHSV(critpartcolor2.Hue, critpartcolor2.Sat, critpartcolor2.Value))})
						until not Killaura.Enabled or killauraparticle.Enabled
						particle.Parent = killauraparticlepart
						critpartcolor.Object.Visible = true
						critpartcolor2.Object.Visible = true
					else
						if killauraparticlepart then 
							killauraparticlepart:Destroy()
							killauraparticlepart = nil
							critpartcolor.Object.Visible = false
						    critpartcolor2.Object.Visible = false
						end
					end
				end
			})
			critpartcolor = Killaura.CreateColorSlider({
				Name = "Crit Particle Color 1",
				Function = function() end,
			})
			critpartcolor2 = Killaura.CreateColorSlider({
				Name = "Crit Particle Color 2",
				Function = function() end,
			})
											...
											if v10.Team ~= lplr.Team then
												while v10 and v10.Character.Humanoid.Health > 0 and v10.Character.PrimaryPart do
													...
													if KillAll.Enabled and TPAuraMethod.Value == "Random" then
														if not getItem("arrow") and not getItem("snowball") and bow and not dead then if dead then return end KillAll.ToggleButton(false) warningNotification("TPAura","Teleporting stopped. Please restock on arrows or snowballs to continue.",5) return end
						...
						Function = function(callback)
							if callback then
								task.spawn(function()
									repeat task.wait() until game:IsLoaded()
									repeat task.wait() until bedwarsStore.matchState ~= 0
										healthbar = lplr.PlayerGui.hotbar["1"].HotbarHealthbarContainer.HealthbarProgressWrapper:FindFirstChild("1") or lplr.PlayerGui:WaitForChild("hotbar"):WaitForChild("1"):WaitForChild("HotbarHealthbarContainer"):WaitForChild("HealthbarProgressWrapper"):WaitForChild("1")
										repeat task.wait()
					...
					playertp = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
						...
						Function = function(callback)
							...
							task.spawn(function()
								...
								local target = nil
								local distance = tprange.Value
		
								for i,v in next, playersService:GetPlayers() do
								if v and v ~= lplr and v.Character and v.Character:FindFirstChildOfClass('Humanoid') and v.Character:FindFirstChildOfClass('Humanoid').RootPart and v.Character.Humanoid.Health ~= 0 and v.Team ~= lplr.Team then
								local plrdist = lplr:DistanceFromCharacter(v.Character:FindFirstChildOfClass('Humanoid').RootPart.CFrame.p)
						...
						warningNotification("7BitchesExploit","Attatching to " ..(GetClosestPlayer().DisplayName or GetClosestPlayer().Name).. "!",5)
						repeat task.wait()
							if closetpmethod.Value == "Cframe" then
						lplr.Character:FindFirstChildOfClass("Humanoid").RootPart.CFrame = GetClosestPlayer().Character:FindFirstChildOfClass('Humanoid').RootPart.CFrame
							...
							if callback then
								task.spawn(function()
									...
									if BedTPMode.Value == "Damage" then
										...
										warningNotification("BedTP","Take damage to teleport.",6.8)
