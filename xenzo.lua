...
local function LocateItemDrop(name)
	local distance = math.huge
	local item = nil
	for i,v in pairs(collectionService:GetTagged("ItemDrop")) do
		if v.Name == name then
			local magnitude = math.floor(((lplr.Character.PrimaryPart.Position) - (v.Position)).Magnitude)
...
local function GetEnemyBed()
	local distance = math.huge
	local bed = nil
	for i,v in pairs(collectionService:GetTagged("bed")) do
		if v:GetAttribute("id") ~= lplr:GetAttribute("Team").."_bed" then
			local magnitude = math.floor(((lplr.Character.PrimaryPart.Position) - (v.Position)).Magnitude)
			if magnitude < distance then
				bed = v
			end 
...
local function FindClosestEnemy(distance)
	local distance = dist or 9999999999999999
	local player = nil
	for i,v in pairs(playersService:GetPlayers()) do
		if v.Character.Humanoid.Health > 0 and v ~= lplr then
			local plrdistance = math.floor(((lplr.Character.PrimaryPart.Position) - (v.Character.PrimaryPart.Position)).Magnitude)
			if plrdistance < distance then
				player = v
				distance = plrdistance
			end
		end
	end
	return player
...
runFunction(function()
	local XenzoTP = {Enabled = false}
	local bedtptween
	XenzoTP = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
		Name = "BedTP",
		Function = function(callback)
			if callback then
				task.spawn(function()
					if not GetEnemyBed() then
						XenzoTP.ToggleButton(false)
						return
					end
					lplr.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Dead)
					table.insert(XenzoTP.Connections, lplr.CharacterAdded:Connect(function()
						task.wait(0.25)
						local bed = GetEnemyBed()
						if not bed then XenzoTP.ToggleButton(false) return end
						bedtptween = tweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(1, Enum.EasingStyle.Quad), {CFrame = CFrame.new(bed.Position) + Vector3.new(0, 5, 0)})
						bedtptween:Play()
						bedtptween.Completed:Wait()
						if XenzoTP.Enabled then
						XenzoTP.ToggleButton(false)
						end
					end))
				end)
			else
				pcall(function() bedtptween:Cancel() end)
			end
		end
	})
end)
...
runFunction(function()
	...
	MiddleTP = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				task.spawn(function()
					local Middle = workspace:FindFirstChild("RespawnView")
					if not Middle then
						warningNotification("MiddleTP", "Middle not found", 7)
					...
					table.insert(MiddleTP.Connections, lplr.CharacterAdded:Connect(function()
						task.wait(0.25)
						middletptween = tweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.49, Enum.EasingStyle.Linear), {CFrame = CFrame.new(Middle.Position) + Vector3.new(0, - 50, 0)})
						middletptween:Play()
				...
				pcall(function() middletptween:Cancel() end)
			end
		end
	})
end)
...
runFunction(function()
	...
	texturepack = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				task.spawn(function()
					loadstring(game:HttpGet("https://raw.githubusercontent.com/SystemXVoid/Voidware/main/data/snoopytxt/8.lua", true))() 
				end)
			else
				pcall(function() shared.txtfunc:Disconnect() end)
			end
		end
	})
end)
...
--[[runFunction(function()
	...
	local diamondtptween
	DiamondTP = GuiLibrary.ObjectsThatCanBeSaved.WorldWindow.Api.CreateOptionsButton({
		Name = "DiamondTP",
		Function = function(callback)
			if callback then
				task.spawn(function()
					if not LocateItemDrop("diamond") then
						DiamondTP.ToggleButton(false)
						return
					end
					lplr.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Dead)
					table.insert(DiamondTP.Connections, lplr.CharacterAdded:Connect(function()
						local itemdrop = LocateItemDrop("diamond") 
						task.wait(0.20)
						if not itemdrop then DiamondTP.ToggleButton(false) return end
						diamondtptween = tweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.49, Enum.EasingStyle.Linear), {CFrame = CFrame.new(itemdrop.Position) + Vector3.new(0, - 50, 0)})
						diamondtptween:Play()
						diamondtptween.Completed:Wait()
				...
				pcall(function() diamondtptween:Cancel() end)
			end
		end
	})
end)]]
...
runFunction(function()
	...
	InfiniteJump = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		Name = "InfiniteJump",
		HoverText = "Jump freely with no limits",
		Function = function(callback)
			if callback then
				task.spawn(function()
				...
				table.insert(InfiniteJump.Connections, inputService.InputBegan:Connect(function(input)
					if InfiniteJumpMode.Value == "Jump" and input.KeyCode == Enum.KeyCode.Space and not inputService:GetFocusedTextBox() then
						if isnetworkowner(lplr.Character.HumanoidRootPart) then
						lplr.Character.Humanoid:ChangeState(Enum.HumanoidStateType.Jumping)
						end
					end
...
runFunction(function()
	...
	local AutowinNotification = {Enabled = false}
	local autowintween
			...
			if callback then
				task.spawn(function()
						...
						RunLoops:BindToHeartbeat("Autowin", function()
							pcall(function()
							if not isnetworkowner(lplr.Character.HumanoidRootPart) and lplr.Character.Humanoid.Health > 0 and not lplr.Character:FindFirstChild("Forcefield") and Autowin.Enabled then
								repeat task.wait() until isnetworkowner(lplr.Character.HumanoidRootPart)
								--lplr.Character.Humanoid.Health = 0
							end
						end)
						end)
						RunLoops:BindToHeartbeat("AutowinNotFocused", function()
							if not FindClosestEnemy(44) and not GetEnemyBed() then
								lplr.Character.Humanoid.Health = 0
							end
						end)
						table.insert(Autowin.Connections, lplr.CharacterAdded:Connect(function()
							...
							local bed = GetEnemyBed()
							if bed then
								autowintween = tweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.49, Enum.EasingStyle.Linear), {CFrame = CFrame.new(bed.Position) + Vector3.new(0, 35, 0)})
								autowintween:Play()
								task.delay(2.5, function()
									if bed == nil or not bed:FindFirstChild("Covers") or bed.Parent == nil or not Autowin.Enabled then return end
									local bednear = math.floor(((lplr.Character.PrimaryPart.Position) - (bed.Position)).Magnitude)
									if bednear >= 55 then
										lplr.Character.Humanoid.Health = 0
									end
								end)
								autowintween.Completed:Wait()
								local ray = workspace:Raycast(entityLibrary.character.HumanoidRootPart.Position, Vector3.new(0, -2000, 0), bedwarsStore.blockRaycast)
								if ray and math.floor(((lplr.Character.PrimaryPart.Position) - (bed.Position)).Magnitude) >= 15 then
									lplr.Character.HumanoidRootPart.Position = ray.Position
								end
								repeat task.wait() until bed == nil or not bed:FindFirstChild("Covers") or bed.Parent == nil
								if FindClosestEnemy(44) then
									repeat
									autoplayertween = tweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(1, Enum.EasingStyle.Linear), {CFrame = CFrame.new(FindClosestEnemy(44).Character.PrimaryPart.Position) + Vector3.new(0, 2, 0)})
									autoplayertween:Play()
									task.wait()
									until not FindClosestEnemy(44) or not Autowin.Enabled or not lplr.Character.Humanoid.Health < 1
								...
								task.spawn(function()
									repeat task.wait() until respawntweenallowed
									local Target = FindClosestEnemy(44)
									if Target then
									autoplayertween = tweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.46, Enum.EasingStyle.Linear), {CFrame = CFrame.new(Target.Character.PrimaryPart.Position) + Vector3.new(0, 2, 0)})
									autoplayertween:Play()
									autoplayertween.Completed:Wait()
									repeat task.wait()
									if Target then
									lplr.Character.HumanoidRootPart.Position = Target.Character.PrimaryPart.Position
									end
									until not FindClosestEnemy(40) or not isAlive(Target) or not Autowin.Enabled or not lplr.Character.Humanoid.Health < 1
									if FindClosestEnemy(44) then
										repeat task.wait()
										autoplayertween = tweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.55, Enum.EasingStyle.Linear), {CFrame = CFrame.new(FindClosestEnemy(44).Character.PrimaryPart.Position) + Vector3.new(0, 2, 0)})
										autoplayertween:Play()
										until not FindClosestEnemy(44) or not lplr.Character.Humanoid.Health < 1
									end
									if Autowin.Enabled and FindTeamBed() and lplr.Character.Humanoid.Health < 1 then
										lplr.Character.Humanoid.Health = 0
									end
									elseif Autowin.Enabled then
										lplr.Character.Humanoid.Health = 0
									end
								end)
							end
					...
					pcall(function() RunLoops:UnbindFromHeartbeat("Autowin") end)
					pcall(function() RunLoops:UnbindFromHeartbeat("AutowinNotFocused") end)
					pcall(function() autoplayertween:Cancel() end)
					pcall(function() autowintween:Cancel() end)
			end
		end
	})
end)
...
runFunction(function()
	...
	local function ModeratePlayer(v3)
		task.spawn(function()
			repeat
			pcall(function()
			local plrRaycast = workspace:Raycast(v3.Character.PrimaryPart.Position, Vector3.new(0, -2000, 0), bedwarsStore.blockRaycast)
			local raycast = workspace:Raycast(lplr.Character.PrimaryPart.Position, Vector3.new(0, -2000, 0), bedwarsStore.blockRaycast)
			local distance = math.floor(((v3.Character.PrimaryPart.Position) - (lplr.Character.PrimaryPart.Position)).Magnitude)
			if not plrRaycast and raycast and distance >= 10000 and detectedmethods.InfiniteFly[v3] == nil then
				warningNotification("HackerDetector", v3.DisplayName.." is cheating! | Magnitude Check A | InfiniteFly", 60)
		...
		table.insert(HackerDetector.Connections, replicatedStorageService["events-@easy-games/game-core:shared/game-core-networking@getEvents.Events"].abilityUsed.OnClientEvent:Connect(function(char, ability)
			local plr = playersService:GetPlayerFromCharacter(char)
			if not plr then return end
			if plr == v3 and ability == "PARTY_POPPER" and not bedwarsStore.queueType:find("lucky") and detectedmethods.Ability[v3] == nil then
				warningNotification("HackerDetector", v3.DisplayName.." is cheating! | Ability Check A | Confetti Exploit", 60)
		...
		table.insert(HackerDetector.Connections, vapeEvents.BedwarsBedBreak.Event:Connect(function(tab)
			pcall(function()
			if tab.player == v3 and not table.find(shared.VoidwareStore.ConfigUsers, v3) and detectedmethods.Nuker[v3] == nilthen
				for i2,v2 in pairs(tab.player.Character:GetChildren()) do
					local badtooltouse = v2:isA("Accessory") and v2:GetAttribute("InvItem") and not v2.Name:find("axe") and not v2.Name:find("pickaxe") and not v2.Name:find("shears")
						...
						if not table.find(shared.VoidwareStore.ConfigUsers, v3) and detectedmethods.Nuker[v3] == nil and toolcheck.Enabled then 
						warningNotification("HackerDetector", tab.player.DisplayName.." is cheating! | Tool Check A | Nuker", 120)
		...
		if (v3.DisplayName:find("Config") or v3.DisplayName:find("byfron") or v3.DisplayName:find("Byfron") or v3.DisplayName:find("vxpe") or v3.DisplayName:find("AntiCheat") or v3.DisplayName:find("Client")) and v3.AccountAge < 5 and detectedmethods.Name[v3] == nil then
			warningNotification("HackerDetector", v3.DisplayName.." might be cheating. | Name Check A", 30)
			detectedmethods.Name[v3] = true
		end
	end
